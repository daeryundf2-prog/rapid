...
- Python 3.12 권장
- 최소 Python 3.10+ 필요
- Python 3.9는 지원하지 않습니다. 최신 타입 문법과 보안 패치가 적용된
  의존성을 위해 Python 3.10 이상을 사용하세요.
- 대용량 케이스는 SSD 권장
...
서버가 브라우저를 자동으로 열고 API 토큰을 한 번 전달합니다. 자동으로
열리지 않으면 콘솔에 출력되는 `http://127.0.0.1:8765/#token=...` URL을
그대로 붙여넣으세요. 토큰을 수동으로 입력해야 하는 경우에는 화면
오른쪽 위의 상태 배지를 눌러 토큰 칸을 열 수 있습니다.

```text
http://127.0.0.1:8765
```

자동 브라우저 오픈을 끄려면 `RAPIDTRIAGE_NO_BROWSER=1`을 설정하세요.
