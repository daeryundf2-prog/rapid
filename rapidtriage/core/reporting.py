    ...
    *,
    ...
    return {
        ...
        "processing": {
            ...
            "resume": processing.get("resume", False) if isinstance(processing, Mapping) else False,
            "reused_output_count": processing.get("reused_output_count", 0) if isinstance(processing, Mapping) else 0,
            "reused_outputs": list(processing.get("reused_outputs", [])) if isinstance(processing, Mapping) and isinstance(processing.get("reused_outputs", []), list) else [],
...
def render_run_markdown_report(report_context: Mapping[str, object]) -> str:
    ...
    lines = [
        ...
        f"- Resume/reuse outputs: {processing['resume']} ({processing['reused_output_count']} reused)",
        ...
        f"- Reused outputs: {', '.join(str(item) for item in processing['reused_outputs']) or 'none'}",
        "",
        "### Processing warnings",
        "",
    ]
