...
def build_core_forensics_accuracy_profiles() -> dict[str, object]:
    ...
    return {
        ...
        "scope": "#1-#30 validation and accuracy reinforcement",
        "profile_count": len(profiles),
        "commercial_gap_ids": [f"#{item['number']}" for item in profiles],
        "status": "accuracy-profiles-ready",
        "release_gate": "Each #1-#30 parser claim must attach pass/fail evidence against its profile before report-grade wording.",
        "profiles": profiles,
    }
...
def build_accuracy_profile(item: Mapping[str, object]) -> dict[str, object]:
    ...
    return {
        ...
        "accuracy_controls": {
            ...
            "hash_required": True,
            "timezone_or_timestamp_semantics_required": number in {1, 3, 4, 6, 7, 8, 9, 10, 11, 12, 13, 15, 16, 17, 18, 20, 22, 23, 24, 26, 27, 29},
            "secret_redaction_required": number in {19, 26, 27, 28, 29, 30},
            "legal_or_authority_gate_required": number in {19, 22, 25, 26, 27, 28, 29, 30},
        ...
        "pass_fail_rules": [
            ...
            "fail when parser output loses record order, source path/hash, record ID, offset, or timestamp semantics required by the profile",
...
def accuracy_profile_for_item(number: int) -> dict[str, object]:
    ...
    raise KeyError(f"unknown #1-#30 forensic accuracy item: {number}")
