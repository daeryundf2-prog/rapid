...
def build_commercial_gap_assessment() -> list[dict[str, object]]:
    return [
        ...
        },
        {
            ...
            "severity": "high",
            "current_status": "EVTX native scanning is partial but now preserves common BinXML scalar values, SIDs, TemplateInstance IDs, message-rendering provenance, and cautious slack/deleted/corrupt record candidate metadata; MFT/USN support includes imports plus bounded native inventory/USN record recovery; registry support includes exports, inventory-level native hive parsing, bounded key-tree rows, value-recovery candidates, and first-pass NTUSER/UsrClass user-activity pivots; SRUDB/Windows.edb include direct ESE header and bounded string-pivot inventory but not full table decoding.",
