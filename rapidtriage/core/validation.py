...
def build_commercial_gap_assessment() -> list[dict[str, object]]:
    return [
        ...
        },
        {
            ...
            "severity": "high",
            "current_status": "EVTX native scanning is partial but now preserves common BinXML scalar values, SIDs, TemplateInstance IDs, message-rendering provenance, native chunk structure rows, and cautious slack/deleted/corrupt record candidate metadata; MFT/USN support includes imports plus bounded native inventory/USN record recovery; registry support includes exports, inventory-level native hive parsing, hbin-aware bounded key-tree rows, key/value recovery candidates, and first-pass NTUSER/UsrClass user-activity pivots; SRUDB/Windows.edb include direct ESE header and bounded string-pivot inventory but not full table decoding.",
            "needed_for_commercial_parity": "Full EVTX BinXML/provider message resource rendering, native Registry hive transaction-log replay, deep NTUSER.DAT/UsrClass.dat binary value decoding and deleted-value testimony validation, SRUDB ESE, Windows.edb ESE, $MFT, $UsnJrnl, JumpList, ShellBags, Amcache, ShimCache, and Prefetch parsers with validation corpora.",
