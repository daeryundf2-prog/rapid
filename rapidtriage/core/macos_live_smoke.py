    ...
    *,
    ...
    payload: dict[str, object] = {
        ...
        "outputs": {
            ...
            "readiness_attachment_script": str(output_dir / "attach-mac-evidence-to-readiness.sh"),
    ...
    write_readiness_attachment_script(payload, output_dir / "attach-mac-evidence-to-readiness.sh")
    return payload
...
def write_readiness_attachment_script(payload: Mapping[str, object], path: Path) -> None:
    attachment = payload.get("readiness_attachment") if isinstance(payload.get("readiness_attachment"), Mapping) else {}
    command = str(attachment.get("cli_command") or "rapidtriage commercial-readiness --json")
    script = "\n".join(
        [
            "#!/bin/sh",
            "set -eu",
            "",
            "# Generated by rapidtriage macos-live-smoke.",
            "# This attaches Mac-first smoke evidence as preparatory proof only.",
            command,
            "",
        ]
    )
