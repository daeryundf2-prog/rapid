...
MEDIA_SUSPICION_PREFIX_BYTES = 2 * 1024 * 1024
MEDIA_SUSPICION_TAIL_BYTES = 512 * 1024
HANGUL_RE = re.compile(r"[\uac00-\ud7a3\u1100-\u11ff\u3130-\u318f]")
EMBEDDED_PAYLOAD_SIGNATURES: tuple[tuple[bytes, str], ...] = (
    (b"PK\x03\x04", "zip-local-file-header"),
    (b"Rar!\x1a\x07", "rar-archive-header"),
    (b"7z\xbc\xaf\x27\x1c", "7zip-archive-header"),
    (b"SQLite format 3\x00", "sqlite-database-header"),
    (b"%PDF-", "pdf-header"),
    (b"MZ", "portable-executable-header"),
)
MEDIA_AUTHENTICITY_TEXT_HINTS: tuple[tuple[bytes, str, str], ...] = (
    (b"stable diffusion", "stable-diffusion-metadata", "ai-generation"),
    (b"comfyui", "comfyui-workflow-metadata", "ai-generation"),
    (b"midjourney", "midjourney-metadata", "ai-generation"),
    (b"dall-e", "dall-e-metadata", "ai-generation"),
    (b"generated by ai", "generic-ai-generation-metadata", "ai-generation"),
    (b"civitai", "civitai-metadata", "ai-generation"),
    (b"photoshop", "photoshop-editing-metadata", "editing-tool"),
    (b"lightroom", "lightroom-editing-metadata", "editing-tool"),
    (b"gimp", "gimp-editing-metadata", "editing-tool"),
)
...
def read_prefix_tail(path: Path, *, prefix_limit: int, tail_limit: int) -> tuple[int, bytes, bytes, int]:
    try:
        size = path.stat().st_size
        with path.open("rb") as handle:
            prefix = handle.read(min(size, prefix_limit))
            tail_offset = 0
            tail = prefix
            if size > tail_limit:
            ...
            return size, prefix, tail, tail_offset
    except OSError:
        return 0, b"", b"", 0
...
def detect_image_end_offset(path: Path, *, file_size: int, prefix: bytes, tail: bytes, tail_offset: int) -> int | None:
    suffix = path.suffix.lower()
    if suffix == ".png":
        marker = b"IEND\xaeB`\x82"
        tail_index = tail.rfind(marker)
        if tail_index >= 0:
            return tail_offset + tail_index + len(marker)
        prefix_index = prefix.rfind(marker)
        if prefix_index >= 0:
            return prefix_index + len(marker)
    if suffix in {".jpg", ".jpeg"}:
        ...
        if prefix_index >= 0:
            return prefix_index + len(marker)
    if suffix == ".webp" and prefix.startswith(b"RIFF") and prefix[8:12] == b"WEBP" and len(prefix) >= 8:
        return min(file_size, int.from_bytes(prefix[4:8], "little", signed=False) + 8)
    if suffix == ".bmp" and prefix.startswith(b"BM") and len(prefix) >= 6:
        return min(file_size, int.from_bytes(prefix[2:6], "little", signed=False))
    return None


def embedded_payload_signature_candidates(
    *,
    prefix: bytes,
    tail: bytes,
    tail_offset: int,
    ...
    windows: list[tuple[bytes, int, str]] = [(prefix, 0, "prefix")]
    if tail_offset:
        windows.append((tail, tail_offset, "tail"))
    for blob, base_offset, scan_window in windows:
        for signature, label in EMBEDDED_PAYLOAD_SIGNATURES:
            start = 0
            while len(candidates) < limit:
                index = blob.find(signature, start)
                if index < 0:
                    break
                absolute_offset = base_offset + index
                if image_end_offset is None:
                    after_image_end = absolute_offset >= 32
                else:
                    after_image_end = absolute_offset >= image_end_offset
                if after_image_end:
                    candidates.append(
                        {
                            "signature": label,
                            ...
                            "after_image_end": after_image_end,
                        }
                    )
                start = index + 1
...
def steganography_suspicion_profile(path: Path, *, width: int | None, height: int | None) -> dict[str, object]:
    file_size, prefix, tail, tail_offset = read_prefix_tail(
        path,
        prefix_limit=MEDIA_SUSPICION_PREFIX_BYTES,
        tail_limit=MEDIA_SUSPICION_TAIL_BYTES,
    )
    if not prefix and file_size == 0:
        return {
            "profile_version": "steganography-suspicion-profile-v1",
            "status": "scan-failed-or-empty",
            "suspicion_level": "unknown",
            "validation_status": "not-run",
            "candidate_only": True,
        }
    image_end_offset = detect_image_end_offset(path, file_size=file_size, prefix=prefix, tail=tail, tail_offset=tail_offset)
    trailing_data_bytes = max(0, file_size - image_end_offset) if image_end_offset is not None else 0
    signature_candidates = embedded_payload_signature_candidates(
        prefix=prefix,
        tail=tail,
    ...
    reasons: list[str] = []
    if trailing_data_bytes:
        reasons.append("trailing-data-after-image-end")
    if signature_candidates:
        reasons.append("embedded-signature-after-image-end")
    prefix_entropy = shannon_entropy(prefix)
    tail_entropy = shannon_entropy(tail)
    if trailing_data_bytes and tail_entropy >= 7.85:
        reasons.append("high-entropy-trailing-window")
    suspicion_level = "high" if signature_candidates else ("medium" if trailing_data_bytes else "none")
    return {
        "profile_version": "steganography-suspicion-profile-v1",
        "status": "completed",
        "scan_strategy": "bounded-prefix-tail-signature-scan",
        "source_format": path.suffix.lower().lstrip("."),
        "file_size": file_size,
        "width": width,
        "height": height,
        "scan_bytes": len(prefix) + (0 if tail_offset == 0 else len(tail)),
        "prefix_entropy": prefix_entropy,
        "tail_entropy": tail_entropy,
        "image_end_offset": image_end_offset,
        "trailing_data_present": trailing_data_bytes > 0,
        "trailing_data_bytes": trailing_data_bytes,
        "embedded_signature_candidates": signature_candidates,
        "suspicion_level": suspicion_level,
        "suspicion_reasons": reasons,
        "candidate_only": True,
        "validation_status": "triage-suspicion-not-proof",
        "not_proof_of": [
            "hidden message",
            "malicious intent",
        ...
        "report_blockers": [
            "validated-stego-tool-diff-required",
            "manual-review-of-trailing-bytes-required",
            "known-answer-stego-corpus-required",
        ],
    }


def media_authenticity_profile(path: Path, *, classification: Mapping[str, object]) -> dict[str, object]:
    file_size, prefix, tail, tail_offset = read_prefix_tail(
        path,
        prefix_limit=MEDIA_SUSPICION_PREFIX_BYTES,
        tail_limit=MEDIA_SUSPICION_TAIL_BYTES,
    )
    scan_windows: list[tuple[bytes, int, str]] = [(prefix.lower(), 0, "prefix")]
    if tail_offset:
        scan_windows.append((tail.lower(), tail_offset, "tail"))
    ai_candidates: list[dict[str, object]] = []
    editing_candidates: list[dict[str, object]] = []
    for blob, base_offset, scan_window in scan_windows:
        for needle, label, category in MEDIA_AUTHENTICITY_TEXT_HINTS:
            ...
            if category == "ai-generation":
                ai_candidates.append(row)
            else:
                editing_candidates.append(row)
    indicators: list[str] = []
    if ai_candidates:
        indicators.append("ai-generation-metadata-candidate")
    if editing_candidates:
        indicators.append("editing-tool-metadata-candidate")
    suspicion_level = "medium" if ai_candidates else ("low" if editing_candidates else "none")
    return {
        "profile_version": "media-authenticity-profile-v1",
        "status": "completed" if file_size else "scan-failed-or-empty",
        "scan_strategy": "bounded-metadata-string-hint-scan",
        "classifier_status": "heuristic-metadata-only",
        "deepfake_detection_status": "not-run",
        "manipulation_detection_status": "metadata-hint-only",
        "classification_hint": dict(classification) if classification else {},
        "ai_generation_metadata_candidates": ai_candidates[:20],
        "editing_tool_metadata_candidates": editing_candidates[:20],
        "manipulation_indicators": indicators,
        "suspicion_level": suspicion_level,
        "candidate_only": True,
        "validation_status": "triage-suspicion-not-proof",
        "report_blockers": [
            "validated-media-authenticity-model-required",
            "model-version-and-threshold-not-available",
            "known-answer-deepfake-manipulation-corpus-required",
        ],
    }


def media_review_risk_flags(
    *,
    decoded: bool,
    ...
    if steganography_profile.get("trailing_data_present"):
        flags.append("trailing-data-after-image-end")
    if steganography_profile.get("embedded_signature_candidates"):
        flags.append("embedded-payload-signature-candidate")
    if authenticity_profile.get("ai_generation_metadata_candidates"):
        flags.append("ai-generation-metadata-candidate")
    if authenticity_profile.get("editing_tool_metadata_candidates"):
        flags.append("editing-tool-metadata-candidate")
...
def build_image_record(path: Path) -> ArtifactRecord:
    ...
    steganography_profile = steganography_suspicion_profile(
        resolved,
        width=parse_int(details.get("width")),
...
def image_tag_suggestions(details: Mapping[str, object]) -> list[str]:
    ...
    if "steganography-suspicion-candidate" in risk_flags:
        tags.append("steganography-review")
    if "ai-generation-metadata-candidate" in risk_flags:
        tags.append("ai-generation-review")
    if "editing-tool-metadata-candidate" in risk_flags:
        tags.append("editing-metadata-review")
    return tags
...
def media_core_accuracy_gates(*, details: dict[str, object], source_path: Path) -> list[dict[str, object]]:
    ...
    if details.get("steganography_suspicion_profile"):
