...
| 검색 / 타임라인 고급 | De-NISTing/Whitelisting | 부분 구현+ | `rapidtriage files --known-good-hash-feed`로 TXT/CSV/JSON 및 NSRL RDS CSV/`NSRLFile.txt`(`SHA-1`, `MD5`, `FileName`, `ProductCode` 등) 피드를 읽어 known-good 파일을 표시하거나 `--hide-known-good`로 숨김. `rapidtriage known-good-index`로 여러 feed/디렉터리를 `known-good-index-v1` JSON으로 정규화해 재사용 가능. 매칭 row는 `known_good_match.source_detail`에 feed/row/hash column/NSRL metadata를 남김. 통합 검색도 `--hide-known-good`/GUI 체크박스로 known-good 파일 hit를 숨기고 `known_good_search_suppression_profile`에 reviewable/suppressed count와 source trace를 남김. Bundled NSRL online updater/full public DB validation은 남음 |
