...
| 검색 / 타임라인 고급 | De-NISTing/Whitelisting | 부분 구현+ | `rapidtriage files --known-good-hash-feed`로 TXT/CSV/JSON/ZIP 및 NSRL RDS CSV/`NSRLFile.txt`(`SHA-1`, `MD5`, `FileName`, `ProductCode` 등) 피드를 읽어 known-good 파일을 표시하거나 `--hide-known-good`로 숨김. `rapidtriage known-good-index`로 여러 feed/디렉터리/ZIP을 `known-good-index-v1` JSON으로 정규화해 재사용 가능. ZIP feed는 member cap, size cap, compression-ratio cap을 적용하고 `archive_member`/`feed_container_format`을 source detail에 남김. 매칭 row는 `known_good_match.source_detail`에 feed/row/hash column/NSRL metadata를 남김. 통합 검색도 `--hide-known-good`/GUI 체크박스로 known-good 파일 hit를 숨기고 `known_good_search_suppression_profile`에 reviewable/suppressed count와 source trace를 남김. Bundled NSRL online updater/full public DB validation은 남음 |
...
| ADS named stream / Zone.Identifier | `ads-stream-candidate`, `ads_review_profile` | `name:stream`, `name:stream:$DATA`, `name.Zone.Identifier` export 파일을 찾아 host-file 존재, stream name/type/family, ZoneId/Referrer/HostUrl, bounded preview, embedded signature, source hex locator, script/exe risk flag를 기록한다. | 원본 NTFS의 native ADS enumeration, `$MFT/$UsnJrnl` STREAM_CHANGE 상관, extraction manifest/trusted parser diff |
| ZIP/RAR/7z archive inventory | `archive-file-inventory`, `archive_review_profile` | ZIP central directory를 추출 없이 읽어 encrypted entry, path traversal, executable/script entry, nested archive, compression ratio를 보여주고 RAR/7z는 metadata-only row로 sandbox/parser 필요성을 표시한다. | sandboxed recursive extraction, password candidate workflow, entry hash manifest, 7z/RAR trusted parser diff |
...
3. `ads-stream-candidate`는 export 파일 기반이다. 원본 NTFS image에서 stream이 실제 존재했는지, 어떤 host 파일과 연결되는지, 언제 생성/변경됐는지는 `$MFT`, `$UsnJrnl`, extraction log와 교차검증해야 한다.
4. `archive-file-inventory`는 extraction 전 안전 metadata view다. 암호 해제 성공, 내부 파일 내용, 악성 여부를 의미하지 않는다.
5. `print-spooler-job`은 document/printer/user 후보와 SHD/SPL companion pair 상태를 분리하지만 아직 full SPL/SHD binary structure decoder는 아니다. 실제 출력된 문서명/소유자/프린터 확정에는 PrintService EVTX와 source document metadata 상관이 필요하다.
6. `third-party-remote-control-artifact`는 session/remote ID/file-transfer 후보를 분리하지만 실제 접속 세션, 원격 ID, 파일 전송 여부 확정에는 제품별 로그 format validation과 계정 attribution이 필요하다.
7. `thumbnail-cache-entry-candidate`와 `thumbnail-cache-media-candidate`는 Explorer cache 안의 bounded source locator를 제공하지만, 실제 사용자가 해당 원본 파일을 열었다는 결론은 LNK/JumpList/ShellBags/MFT/USN 등 별도 실행/열람 흔적과 함께 검증해야 한다.
...
1. `tests/test_rapidtriage_generic_documents.py`가 `generic-documents` collector 노출, OOXML 문서 메타데이터/매크로/external relationship 후보, Sticky Notes live row/schema profile/recovery candidate, 로컬 LLM 모델 inventory, desktop AI message row 후보를 검증한다.
...
| capability | 새 artifact row | 구현 내용 | 남은 상용급 보강 |
| --- | --- | --- | --- |
| Windows Timeline ActivitiesCache | `activities-cache-db`, `activities-cache-row-candidate`, `notification-row-candidate` | 기존 Windows system collector의 `ActivitiesCache.db`/notification DB read-only SQLite schema inventory를 visible capability와 연결하고, normalized timeline sample을 row-level artifact로 승격해 app/text/time/source locator/review profile을 GUI에서 직접 검색·리뷰할 수 있게 했다. | ActivitiesCache 테이블별 full row semantics, app/document/URL attribution, deleted state 검증 |
...
4. Python/JS visible capability status는 ActivitiesCache row 후보, BITS, WebCacheV01, desktop cloud sync DB를 `부분 구현`으로 올리고 실제 artifact type terms를 추가했다.
...
1. `activities-cache-row-candidate`와 `notification-row-candidate`는 schema-guided timeline row 후보이다. Windows build별 ActivitiesCache/Notification table semantics와 deleted state 검증 전에는 최종 사용자 행위로 단정하면 안 된다.
...
## 34. 2026-05-14 구현 반영: 미디어 EXIF 지도/스테가노그래피/조작 의심 프로필

이미지 갤러리 기능은 해시, 썸네일, perceptual hash, OCR sidecar 중심이라 “위치, 숨은 데이터, 조작 의심”이 GUI에서 별도 queue로 드러나지 않았다. 이번 라운드에서는 이미지 row에 EXIF GPS map pivot과 bounded triage 프로필을 추가해 분석관이 원본 파일을 열기 전에 위치/의심 후보를 빠르게 선별할 수 있게 했다.

부분 구현으로 승격한 capability:

| capability | 새 필드 / risk flag | 구현 내용 | 남은 상용급 보강 |
| --- | --- | --- | --- |
| 사진 EXIF GPS 지도 | `exif_gps_profile`, `exif_map_review_profile`, `exif-gps-location-candidate`, `exif-datetime-candidate` | 이미지 EXIF GPS IFD에서 latitude/longitude/altitude/datetime/direction/map datum 후보를 정규화하고 `source-map` locator, timeline join hint, report citation blocker를 생성한다. | EXIFTool/native-tool diff, metadata tamper 검증, device clock/timezone 검증, offline map UI E2E |
...
1. `tests/test_rapidtriage_media_image.py::RapidTriageExifGpsProfileTests::test_exif_gps_profile_decodes_map_marker_with_fake_pillow`가 EXIF GPS 좌표/고도/시간을 map marker와 risk flag로 정규화하는지 검증한다.
2. `tests/test_rapidtriage_media_image.py::test_media_image_artifacts_collect_dimensions_hashes_and_similarity_bucket`가 trailing ZIP signature가 붙은 PNG fixture를 정상 이미지로 decode하면서도 `steganography_suspicion_profile`과 risk flag가 생성되는지 검증한다.
3. visible capability와 workbench config는 세 capability를 `목록화`에서 `부분 구현`으로 올리고 실제 output field/risk flag term을 검색 가능한 기능 카드에 추가했다.
4. 사용자 가이드와 parser coverage는 이 기능이 “탐지 확정”이 아니라 review pivot임을 명시한다.

중요한 제한:

1. EXIF GPS는 파일에 기록된 metadata 후보일 뿐이며, 사용자가 실제 위치에 있었다는 단독 증거가 아니다.
2. trailing data는 hidden payload의 후보일 뿐이며, 정상 metadata/도구 잔여물일 수도 있다.
3. AI/편집 도구 metadata 문자열은 생성/조작 가능성의 단서이지 deepfake 판정이 아니다.
4. 보고서에 쓰려면 원본 hash, offset, 수동 확인, 외부 검증 도구 결과, 오탐 검토가 함께 있어야 한다.
...
| GUI review 경고 | `renderSqliteSidecarState`, `sqlite-sidecar-card`, `/source-sqlite-wal-preview` | SQLite preview 상단에 WAL/SHM/journal 감지 여부와 `rapidtriage sqlite-wal-preview` 후속 명령을 보여주고, GUI 버튼으로 bounded WAL preview를 같은 viewer 안에 렌더링한다. | browser E2E, trusted SQLite recovery tool diff, deleted-row recovery |
...
3. `tests/test_rapidtriage_web_static.py::test_workbench_review_queue_and_schema_visibility_contracts`가 `renderSqliteSidecarState`, `sqlite-sidecar-card`, `sqlite-wal-preview`, `/source-sqlite-wal-preview`, `estimated_frame_count`가 GUI 정적 계약에 포함되는지 확인한다.
4. `tests/test_rapidtriage_api.py::test_source_sqlite_wal_preview_endpoint_links_sidecar_review_to_gui`가 source viewer에서 같은 DB의 WAL preview API를 열고 frame/page/hash 후보를 반환하는지 확인한다.
