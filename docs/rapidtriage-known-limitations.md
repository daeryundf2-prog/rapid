...
- Native binary EVTX decoding, full Prefetch file-reference decoding, full Jump List destination parsing, direct SRUDB parsing, full mobile extraction imports, and live cloud acquisition are roadmap items. EVTX-oriented XML/JSON/JSONL/CSV exports from tools such as EvtxECmd, Hayabusa, Chainsaw, or Velociraptor can be imported and normalized; Prefetch SCCA header/version/executable hints, common-version run counts, last-run timestamps, and referenced-path pivots are parsed when available.
