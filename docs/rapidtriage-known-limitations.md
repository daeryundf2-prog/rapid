...
- Native binary EVTX decoding, binary Prefetch run-count parsing, full Jump List/LNK destination parsing, direct SRUDB parsing, full mobile extraction imports, and live cloud acquisition are roadmap items. EVTX-oriented XML/JSON/JSONL/CSV exports from tools such as EvtxECmd, Hayabusa, Chainsaw, or Velociraptor can be imported and normalized; Prefetch SCCA header/version/executable hints are parsed when available.
