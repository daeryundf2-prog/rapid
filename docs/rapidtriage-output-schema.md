...
AI conversation rows may include `ai_transcript_schema_validation_manifest`, a stable hashable #21 manifest that separates detected service counts, service/schema validation state, source storage coverage, JSON Pointer locator coverage, Q/A pairing quality, candidate manifest linkage, reportability limits, and exact blockers for service-side export, schema-version, deleted-fragment, and trusted-diff validation. Service export conversation rows can carry per-row `source_json_pointer`, `source_locator_type`, `text_sha256`, `row_citation_hash`, and `export_schema_profile` fields so a GUI can jump from a Q/A candidate back to the original JSON/JSONL export object before report use. AI usage and conversation rows may also include `ai_transcript_analyst_review_profile`, a compact GUI/report card with Q/A counts, completeness score, source manifest hashes, "not complete transcript" warnings, service-export validation blockers, and correlation targets such as raw browser storage, service exports, downloads, documents, and cloud exports.
Browser history/download and storage inventory rows may also include `browser_storage_depth_manifest`, a stable hashable #19 manifest that separates cache/session/extension/sync/cookie/credential inventory scope, native decode depth, legal-scope review controls, citation refs, reportability limits, and exact blockers. Storage inventory rows may also include per-store `source_context_profile` objects with bounded extension manifest, cache signature, SQLite schema/header, and session structure hints plus a `source_context_profile_hash` for citation. This prevents cache/session/extension/sync inventory rows from being overstated as complete browser-version schema decoding.
Browser history/download rows may also include `browser_timeline_depth_manifest`, a stable hashable #20 manifest that separates unified visit/download timeline scope, timestamp/source-index integrity, browser-family native depth, source citation refs, Safari/deleted-state limits, and exact blockers. On macOS Safari, download rows can be sourced from LaunchServices `QuarantineEventsV2` and carry `download_evidence=macos-quarantine`, the quarantine source table/row ID, source path/hash, and agent provenance. This prevents normalized timeline rows from being overstated as a complete cross-browser chronology without trusted diff and known-answer validation.
...
Event log artifact rows use open-ended `details` so parser-specific fields can evolve. Current normalized event rows include `event_id`, `provider_name`, `channel`, `event_family`, typed user/process/network pivots, `event_message`, `message_rendering`, and `event_semantics_profile`. The semantics profile is a GUI/report aid that records analyst severity, review questions, primary pivot fields, populated source values, risk tags, correlation targets, and validation requirements for high-value Windows events. The `message_rendering` object records whether the message came from an imported export field, a provider manifest/catalog template, a RapidTriage built-in fallback template, or an unresolved native provider template; manifest/catalog rendering can preserve template field specs, `valueMap`/`bitMap` mapped display labels, raw source values in `used_fields`, rendering confidence, provenance, and limitations. Native EVTX rows can also include `evtx_record_provenance`, `evtx_native_parse_profile`, `evtx_binxml_grammar_coverage_profile`, `evtx_binxml_grammar_coverage_profile_hash`, `evtx_message_rendering_profile`, `evtx_validation_checks`, `evtx_recovery_context`, and `evtx_report_citation_manifest`; invalid record headers are emitted as `eventlog-record-candidate` rows with offset, integrity, confidence, and caution-label fields. Native BinXML value fields now include best-effort CDATA, character-reference, entity-reference, processing-instruction target, processing-instruction data token rows, and bounded nested `BinXmlType` payload profiles (`evtx-nested-binxml-value-v1`) with payload hash, parser status, field map, token counts, and previews when present, while keeping unsupported-token counts explicit. The grammar coverage profile records observed token names/counts, unsupported token names/counts, decoded/unknown/unvalidated value types, nested BinXML counts, warnings, and reportability blockers so analysts can filter EVTX rows by parser-depth risk before citation. The EVTX citation manifest preserves record offset/hash locators, rendered-message hash, bounded EventData field citations, validation-matrix pass/fail IDs, reportability blockers, and a stable manifest hash for review/report handoff. Parseable slack/deleted native rows and invalid recovery candidates can include `evtx_recovery_report_citation_manifest`, which preserves source hash, candidate offset/size/hash, chunk/free-space context, recovery class, required independent checks, caution labels, and a stable manifest hash while forcing `evtx-recovery-triage-pivot-only` reportability until known-answer/deleted-corrupt corpus validation is attached. Recovery corpus diff normalization reads both top-level fields and nested `evtx_recovery_evidence`/`evtx_recovery_context` values so validation can compare real generated rows without hand-flattening. Native chunk structure rows are emitted as `eventlog-chunk` with `evtx_chunk_header`, `evtx_chunk_integrity`, slack bounds, and checksum-observation fields.

Registry artifact rows also keep parser-specific `details` open-ended. Native hive rows can include `registry-key-tree-node` for best-effort key reconstruction, `registry-key-recovery-candidate` for free/deleted `nk` key cells, and `registry-value-recovery-candidate` for free/deleted `vk` value cells, with source offsets, hbin scan method, allocation state, confidence, validation guidance, transaction-log evidence, `registry_transaction_replay_profile`, and cautious recovery metadata. Key-tree rows, deleted/free-cell rows, and user-activity rows can also include `registry_analyst_review_profile`, which gives GUI/report consumers severity, review questions, primary pivots, populated source values, correlation targets, validation requirements, and commercial blockers. Key-tree rows also carry `registry_subkey_list_profile`, `registry_value_list_profile`, and `registry_key_tree_reconstruction_profile` so reviewers can see the exact `lf`/`lh`/`li`/`ri` subkey-list cells, value-list offsets, decoded child/value counts, list-cell hashes, parent-chain/root reachability, backlink consistency, and bounded reconstruction blockers instead of relying only on a final path string. Key-tree and deleted-cell rows can include `registry_report_citation_manifest` plus `registry_report_citation_manifest_hash`, which preserves hive hash, cell offset, parent cell offset, subkey/value ownership counts, key/value locator, transaction-log context, recovery validation state, validation-matrix pass/fail IDs, reportability blockers, and a stable manifest hash for review/report handoff. Deleted-cell rows also carry `registry_recovery_identity_profile`, allocator-context hashes, allocator-neighbor-context hashes, and recovery identity hashes so second-parser/oracle diffs can prove the exact offset/signature/size/allocation/name/data candidate being reviewed. The registry key-tree diff helper compares nested generated artifact rows against trusted exports using path, cell offset, parent offset, subkey names, value names, linked child/value counts, last-write time, root reachability, and backlink consistency. The deleted-cell diff helper compares nested generated rows against oracle rows using offset, candidate class, cell signature, cell size, hbin offset, allocation status, name, value type/size, data-preview hash, parent path/confidence, allocator hashes, and recovery identity hash; commercial-grade evidence is blocked if required oracle fields are missing. Adjacent `LOG1`/`LOG2` files are now hashed, header-inspected for recognized transaction-log signatures such as `HvLE`/`HvLG`, and summarized by `registry-transaction-replay-validation-v1` without applying replay; that profile records hive dirty/sequence state, log sequence rows, complete-pair status, replay preflight readiness, trusted diff required fields (`log1_sha256`, `log2_sha256`, offsets, replayed key/value locators), blockers, and a stable profile hash that is copied into registry citation manifests. Deleted/free-cell profiles include false-positive controls, allocator neighbor context (`registry-allocator-neighbor-context-v1`), previous/next scanned cell summaries, same-hbin checks, gap-to-next-cell hints, and do-not-report-as-fact wording until independent offset/corpus validation is attached.

Windows commercial-readiness profiles are intentionally emitted inside artifact `details` instead of being kept only in documentation. Current Windows rows may include `registry_user_activity_profile`, `sam_security_system_deep_parser_profile`, `mft_full_parser_profile`, `usn_journal_replay_profile`, nested `execution_artifact_validation_profile` objects, `execution_analyst_review_profile`, `windows_search_analyst_review_profile`, `ntfs_analyst_review_profile`, `jumplist_analyst_review_profile`, `shellbag_analyst_review_profile`, `prefetch_analyst_review_profile`, `lnk_analyst_review_profile`, `system_source_citation_profile`, `system_analyst_review_profile`, and `browser_analyst_review_profile`. These profiles record the normalized row contract, decoded component flags, source provenance, reportability decision, large-data controls, commercial blockers, and exact checks required before court/report-grade use. The analyst review profiles are optimized for GUI/report handoff: they expose severity, evidence interpretation, "not proof of" warnings, analyst questions, primary pivots with populated source values, correlation targets, risk tags, failed validation checks, and commercial blockers for weak or partially decoded Windows artifacts. Windows execution review profiles also embed `native_depth_profile` plus `native_depth_profile_hash`, a stable #7~#10 parser-depth contract for Amcache, ShimCache/AppCompatCache, BAM/DAM, and SRUM rows that records current decode level, OS-version native requirements, trusted-diff fields, blocked native decode gates, validation matrix pass/fail IDs, and reportability decisions.
...
Mobile, iOS, Android, and messenger rows for #26~#35 may also include `mobile_schema_compatibility_matrix` plus `mobile_schema_compatibility_matrix_hash`. Source-level vendor rows expose the same object as `mobile_vendor_schema_compatibility_matrix`; KakaoTalk/WhatsApp/Telegram/Signal/LINE/Discord/Instagram/WeChat rows expose it as `messenger_schema_compatibility_matrix`; iOS backup/keychain rows expose it as `mobile_backup_schema_compatibility_matrix`. The matrix is a hashable `mobile-schema-compatibility-matrix-v1` contract with observed/supported vendor families, vendor artifact mappers, iOS backup/keychain rows, Android backup/app-package rows, messenger service rows, item numbers, observed versions, source hashes, trusted-diff requirements, reportability blockers, and `commercial_grade_ready=false` until version fixtures and external trusted diffs are attached.
...
Cloud API collection requests may include `cloud_api_pagination_execution_profile` (`cloud-api-pagination-execution-v1`) when a manifest declares an implemented bounded pagination strategy such as `next_link_field`. The profile records executed page counts, per-page response sidecar paths and SHA-256s, attempts, response status codes, hashed next-page cursors, hashed delta/sync tokens, profile hash, and `raw_tokens_serialized=false`. Acquisition manifests and report-grade validation plans carry the pagination profile hash and still mark provider-native pagination/delta validation as a blocker until a trusted provider or known-answer corpus proves the API semantics.

Search analysis output for #46~#52 and #60 may include `analysis_analyst_review_profile`, a top-level workbench card that summarizes match, cluster, entity, graph, timeline, workbook, duplicate, review-assignment, and compare-candidate counts with review entrypoints, top source pivots, non-claims, correlation targets, and report blockers. The same payload may include `analysis_review_workflow_manifest` (`search-analysis-review-workflow-manifest-v1`) with bounded reviewer assignment queues, source-viewer locators, A/B/C compare candidate sets, large-data caps, and explicit Case DB persistence requirements. These objects are intentionally review-routing layers: reports should cite the underlying source rows and saved review decisions, not the analysis card or generated assignment manifest by itself.

Completed web/API runs may also expose `GET /api/runs/{run_id}/viewer-workflow-validation`, which returns `run-viewer-workflow-validation-v1` for #51~#60. This run-level payload is a bounded source-viewer route audit: it records candidate source files, viewer family counts, per-route coverage for preview/hex/SQLite/email/image/media/OCR/translation paths, source-locator checks, large-data controls, browser E2E requirements, item coverage, commercial blockers, and a stable `manifest_hash`. It is a QC/routing proof for the UI and analyst workflow, not a commercial-grade certification; browser route traces, trusted viewer corpora, persistent review conflict tests, and real large-file performance traces remain required blockers before report-grade or commercial-grade claims.

Windows OS/account, execution, and Windows Search rows use the same open-ended `details` model. OS/account rows may include account lifecycle, service configuration, mounted-device, LSA-sensitive-location, privilege-assignment, exported group-membership hint, SAM account-candidate, SAM group-candidate, and profile summary records; account lifecycle rows can include SAM F/V binary presence metadata, first-pass SAM F timestamp/RID/UAC candidates, SAM V UTF-16 string candidates, SAM V offset/length layout string candidates such as user name, full name, comments, home/profile paths, validation checks, and commercial-readiness blockers. Account lifecycle rows also include `sam_security_context_manifest`, a stable hashable citation package for normalized group/privilege/service/LSA context rows with per-row hashes, account identity, source hash, secret-redaction policy, reportability blockers, and Case DB indexing safety. #6 account, group, privilege, LSA secret-metadata, and native SAM candidate rows also include `sam_security_system_row_manifest` plus `sam_security_system_row_manifest_hash`, which record stable row identity, source hashes, required trusted-diff fields, validation summaries, redaction policy, and reportability blockers; they now also include `sam_security_system_validation_profile` plus `sam_security_system_validation_profile_hash`, a stable validation gate that binds the row to its SAM F/V candidate status, alias-member binary reconstruction status, SECURITY secret authority/redaction gate, trusted-diff required fields, pass/fail validation IDs, and domain/transaction-log blockers. `os-account-trusted-diff-v1` exposes `field_coverage` and refuses commercial evidence when required account/group/privilege/secret fields are absent. SECURITY/LSA rows may include exported secret value names, value types, byte counts, hashes, entropy, and timestamp candidates without decrypting secrets. Exported group-membership rows include member SID/name lists, source typing, count semantics, validation checks, and native SAM alias-member blockers. Execution rows may include Amcache export/native-hive candidates, bounded native Amcache nearby-string row clusters with source offsets, SHA1 candidates, timestamp candidates, and metadata candidates; Amcache rows now include `amcache_schema_version_profile`, `amcache_row_manifest`, `amcache_row_manifest_hash`, and `amcache_report_citation_manifest` with source hash, source key/offset, normalized path/hash identity, schema-family hint, timestamp semantics, row-cluster locator, required trusted-diff coverage, reportability blockers, and explicit `standalone_execution_proof=false`. Execution rows may also include ShimCache caveated entries, native SYSTEM hive ShimCache/AppCompatCache bounded path clusters with `cache_order`, `source_offset`, nearby timestamp candidates, and not-proof-of-execution warnings; ShimCache rows now include `shimcache_report_citation_manifest` with source hash, source key/offset, normalized path identity, cache-order or timestamp citation, native row-cluster locator, OS-build layout validation blockers, and explicit `standalone_execution_proof=false`. BAM/DAM rows include `bam_dam_row_manifest`, `bam_dam_row_manifest_hash`, and `bam_dam_report_citation_manifest` with source hash, ControlSet/source key/source offset, SID, device path, timestamp semantics, native row-cluster locator, required trusted-diff coverage, binary FILETIME row validation blockers, and explicit correlation-before-testimony wording. SRUM rows now include `srum_report_citation_manifest` for source-tool imports, database inventory, string pivots, table candidates, and row candidates; it preserves source hash, table/app identity, counter names, timestamp semantics, row-cluster/source-offset locator, native ESE row-decoding blockers, and `standalone_execution_proof=false`. These Amcache/ShimCache/BAM/DAM/SRUM rows also emit `execution_analyst_review_profile` so the GUI can show the right severity, pivots, correlation checklist, and caveat wording without treating a weak execution-related artifact as final proof; inside that profile, `native_depth_profile` gives a single hashable parser-depth summary with source locator, current decode depth, OS-version native requirements, trusted-diff required fields, blocked native decode gates, validation IDs, and next validation steps. Execution rows may also include PowerShell history commands, SRUM imports, SRUDB string pivots, native SRUDB validation metadata, SRUM table-family candidates, and bounded `srum-row-candidate` string-cluster rows with `nearby_string_count`, `nearby_offsets`, row-cluster string samples, field-presence profiles, validation checks, and commercial-readiness blockers where native decoding is not yet report-grade. Windows Search EDB rows include `windows_edb_report_citation_manifest`, `windows_edb_report_citation_manifest_hash`, `edb_semantics_warning`, `windows_edb_trusted_diff_contract`, and `windows_search_analyst_review_profile` for source-tool exports, database inventory, string pivots, page candidates, table candidates, and row candidates; they preserve source hash, path/URL/content identity, page locator, table/deleted-state/timestamp semantics, row-cluster hints, not-proof-of warnings, trusted-diff compare fields, and native ESE row/deleted-state validation blockers. Windows Search EDB row candidates may include page-local `page_offset`, `page_sha256`, `page_table_marker_hits`, `field_presence_profile`, and page-local correlation flags; these are source-citation aids, not full native ESE row decoding.
SRUM #10 rows also carry `srum_report_citation_manifest_hash` and a trusted-diff contract that requires `source_format` and `semantics_warning` coverage before a SrumECmd/libesedb-style comparison can be treated as report-grade evidence. The manifest captures source key/offset, table/app identity, timestamp source, individual counter values, network profile/interface LUID, row-cluster/source-offset locators, and explicit caveats so native SRUDB string clusters remain distinguishable from decoded ESE rows.
Windows system rows for Task Scheduler, Defender support logs, Firewall logs, WER reports, and WMI repository files may include `system_source_citation_profile`, `system_deep_parser_manifest`, and `system_analyst_review_profile`, separating normalized semantics, line/offset source citations, risk/review pivots, validation matrix pass/fail IDs, native depth capabilities, citation refs, analyst questions, reportability limits, and the exact cross-artifact correlation blockers required before report-grade claims.

ShimCache rows for #8 now additionally expose `shimcache_layout_profile`, `shimcache_row_manifest`, and `shimcache_row_manifest_hash`. These fields preserve source-viewer locators, path/cache-order/source-offset identity, timestamp and OS-build layout caveats, stable row hashes, required trusted-diff field coverage, and explicit `standalone_execution_proof=false` so GUI/report consumers can use ShimCache as a program-presence/cache-order lead without overstating execution.

BAM/DAM rows for #9 now additionally expose `bam_dam_row_manifest` and `bam_dam_row_manifest_hash`. These fields preserve ControlSet/source-key/source-offset identity, SID, device path, timestamp semantics, source-viewer locators, stable row hashes, required trusted-diff field coverage, and explicit `standalone_execution_proof=false` so GUI/report consumers keep recent-execution pivots tied to corroboration requirements.

Windows system rows may also include Explorer cache, Activities/Notifications/UWP, web-server log, and webshell triage records. `webshell-source-candidate` rows can include `webshell_semantic_profile`, source-code evidence spans, external rule sidecars, IIS site correlation, web-log correlation, filesystem/log timeline correlation, and `webshell_report_citation_package`. The citation package is a bounded review/report handoff with source hashes, text locators, rule sidecar hashes, IIS/log/timeline references, a stable manifest hash, and explicit blockers. Its allowed use is `webshell-triage-correlation-package`; it must not be described as report-defensible malware attribution until trusted rule-pack diff, full server-log corpus, MFT/USN timeline correlation, and manual malware review evidence are attached.

Windows filesystem rows may include MFT/USN import records and native `$MFT`/`$J` inventory. Native MFT rows can include update-sequence validation, common attribute summaries, FILETIME timestamp validation, resident data hashes, bounded nonresident `$DATA` runlist preview decoding, bounded `$ATTRIBUTE_LIST` extension-record resolution when referenced extension records are present in the same bounded scan window, bounded parent-chain path candidates when parent records are present in the scanned window, validation checks, parser confidence, and `commercial_grade_ready=false` blockers because full-volume attribute-list/data-run/path reconstruction still requires dedicated validation. The bounded attribute-list profile records resolved/unresolved entry counts, extension record numbers, sequence-match status, matching extension attribute counts, a profile hash in `mft_parser_depth_manifest`, and blockers for trusted diff/full-volume resolution. Native USN rows can include bounded scan metadata, record cursor/next-cursor offsets, v2/v3 reference metadata, filename UTF-16 validation, v4 extent offset/length previews, bounded MFT path correlation when a USN FRN or parent FRN matches the scanned MFT cache, journal-level `bounded_mft_replay_preview` counts/path samples, `mft_bounded_path_cache_profile` cache quality counts/warnings, `usn_path_reliability_profile` correlation/cache confidence and review priority, `usn_state_replay_validation_profile` when a trusted export diff is attached, `rename_pair_preview` OLD/NEW candidate pairs, `delete_lifecycle_preview` create/delete candidate lifecycles, `bounded_state_replay_preview` create/rename/delete state-transition samples, `timeline_review_candidates` rename/create/delete timestamp pivots, large-record sizing, timestamp ranges where available, validation checks, and `commercial_grade_ready=false` blockers because full replay/correlation and large-corpus pagination validation still require dedicated validation. Rename-pair path candidates prefer parent-cache plus the USN OLD/NEW filename for the event-time path, while preserving `old_frn_path_correlation`/`new_frn_path_correlation` so reviewers can see the bounded MFT cache evidence separately from the event-name path. The bounded state replay sorts by USN/cursor and applies create, rename, and delete effects to a limited in-memory FRN path state; each transition carries `timeline_type`, `reportability`, `validation_required`, and blocker fields so timeline/report output keeps the same caution instead of presenting a replay preview as fact. `usn_state_replay_validation_profile` deliberately separates record-level trusted parser diff success from state-machine replay validation, so a passing UsnJrnl2Csv/MFTECmd row diff is not overstated as a validated rename/delete state replay. Use `build_usn_state_replay_trusted_diff` to compare nested RapidTriage state transitions against known-answer or trusted replay transition rows before treating the state-machine output as validated. Native `mft-record` and `usn-record` rows also include `ntfs_report_citation_manifest`, a stable hashable source-citation object containing source SHA256, row identity, byte offset/cursor, validation pass/fail IDs, viewer entrypoints, reportability limits, and record-specific refs such as MFT `$FILE_NAME`/`$DATA`/`$ATTRIBUTE_LIST` evidence or USN reason/rename/delete/v4-extent/path-correlation refs.
