...
| #53 Hex viewer | Source preview emits bounded hex rows, offsets, preview hashes, byte-search citations, bounded `/source-hex-range` citation packages with range hashes, trusted offset manifest gates, and #53 `core_accuracy_gates`. | Interactive full-file jump UI, trusted offset manifest diff, and byte-level citation package validation against an external oracle. |
| #54 SQLite viewer | Source preview opens SQLite read-only and emits schema/table/column/index/row metadata, API-backed table pages, restricted contains filters, trusted query/schema diff gates, and #54 `core_accuracy_gates`. | Deleted-row/WAL validation, browser E2E pagination proof, trusted sqlite3 query/schema diff, and large database corpus. |
| #55 Email viewer | Source preview emits message threads, message order, participants, headers, attachment hash inventory, bounded attachment citation packages, trusted thread/export diff gates, and #55 `core_accuracy_gates`. | Native PST/OST/MSG conversations, deleted items, native mailbox attachment extraction, trusted mail-client thread export diff, and mailbox corpus validation. |
| #56 Image gallery | Image rows/previews preserve dimensions, hashes, thumbnails, perceptual buckets, folder gallery pages, bucket filters, tags/report hints, trusted image gallery manifest gates, and #56 `core_accuracy_gates`. | Dedicated virtualized gallery UI, persistent tags, ML similarity, sensitive/deepfake classifier validation, and trusted image manifest diff. |
| #57 Media transcript | Media preview preserves metadata, hashes, transcript sidecars, cue timestamps, bounded cue citation packages, warnings, trusted transcript cue/alignment gates, and #57 `core_accuracy_gates`. | Safe playback sandbox, ASR execution, waveform/thumb previews, trusted cue/alignment diff, and transcript alignment corpus. |
| #58 OCR queue | OCR queue/manifests and source preview queue pages preserve queue state, sidecars, hashes, retry handling, metadata, trusted OCR engine/sidecar gates, and #58 `core_accuracy_gates`. | Native OCR worker execution, engine logs, trusted engine/sidecar diff, editable web queue state, and Case DB persistence. |
| #59 Korean OCR/translation | OCR/media flows preserve Korean hints, quality metrics, translation sidecars, confidence/engine metadata, source-viewer side-by-side OCR/translation review packages, trusted Korean OCR/translation gates, and #59 `core_accuracy_gates`. | OCR calibration, trusted Korean OCR review diff, certified translation workflow, and reviewer signoff persistence. |
| #60 Search dedup | Search analysis emits duplicate fingerprints, group counts, representative links, hidden duplicate counts, source/path references, collapse review profiles, web analysis cards, trusted duplicate manifest gates, and #60 `core_accuracy_gates`. | Persistent Case DB suppression workflow, fuzzy text near-duplicate grouping, trusted duplicate manifest diff, and media duplicate validation. |
