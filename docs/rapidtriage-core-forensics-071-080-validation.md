...
Passing this manifest promotes #71 through #80 to the internal `validated` maturity stage only. These features remain `commercial_grade_ready=false` until process isolation, OS-level resource controls, trusted manifests, browser e2e, large-case, and independent validation evidence is attached.
