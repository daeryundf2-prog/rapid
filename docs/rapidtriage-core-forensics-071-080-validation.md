...
| #76 File hash cache | Hash metadata records MD5/SHA1/SHA256 support, path-size-mtime-inode cache keys, hit/miss/invalidation counters, `hash-cache-manifest-v1` entries with hashed paths and manifest hash, stale same-path invalidation policy, trusted hash-cache manifest diff helpers/gates, and #76 `core_accuracy_gates`. | Persistent content-addressed cache, trusted hash-cache manifests, and large-case hit-ratio validation. |
| #77 Duplicate detection | File scans record same-size buckets, bounded SHA256 duplicate groups, stable group IDs/fingerprints, representative paths, not-suppressed analyst-review policy, `duplicate-content-manifest-v1` manifest hashes, trusted duplicate-manifest diff helpers/gates, and #77 `core_accuracy_gates`. | Fuzzy text/media duplicate detection, trusted external duplicate group manifests, and analyst suppression workflow validation. |
