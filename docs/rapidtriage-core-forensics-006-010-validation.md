...
The #6 path also exposes `os-account-trusted-diff-v1`. It compares RapidTriage account, group, privilege, and secret-redaction rows with trusted SAM/SECURITY/SYSTEM parser output by user/RID/SID, UAC flags, group names, privileges, and protected-secret redaction state. Without a clean trusted diff, account rows remain triage pivots and `sam-security-system-trusted-diff-required` remains a commercial blocker.
...
The #7~#10 path also exposes `execution-artifact-trusted-diff-v1`. The helper compares Amcache, ShimCache, BAM/DAM, and SRUM rows with trusted parser exports such as AmcacheParser, AppCompatCacheParser, ShimCacheParser, RECmd, SrumECmd, or Velociraptor by executable/app path, timestamp, SHA1, user SID, counter hash, and semantics warning. This keeps useful execution pivots usable for triage while preventing standalone report-grade execution claims until parser-output diffs and reviewer sign-off are attached.
