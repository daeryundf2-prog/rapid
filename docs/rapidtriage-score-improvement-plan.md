...
- Add high-value Windows parser tests and normalized outputs for Event Logs, Registry hives, Prefetch, LNK, Jump Lists, ShellBags, USB history, SAM/SYSTEM/SECURITY, Amcache/ShimCache/BAM, and SRUM/EDB where feasible. Status: XML/JSON/EventLog exports, strengthened partial native EVTX record scanning with record hashes/integrity/sequence/indicator pivots plus chunk structure rows, `.reg` Registry/USB/Shellbags, native registry hive inventory/string pivots plus hbin-aware bounded `nk`/`vk` cell candidate rows, key-tree rows, separate deleted/free key/value candidates, SAM account/RID candidates, native SAM group/alias key candidates, exported group-membership rows, service/mounted-device/LSA/privilege rows with non-decrypting SECURITY value metadata, Amcache export/native string pivots, ShimCache/BAM caveated rows, Prefetch inventory, Recent/LNK/JumpList inventory, Task Scheduler XML, Defender MPLog, Firewall W3C, WER, Zone.Identifier sidecar, browser fixture coverage, and direct SRUDB/Windows.edb ESE header/string-pivot inventory plus SRUDB native validation, bounded row candidates, and SRUDB/Windows.edb table/path/URL/content candidate rows with explicit commercial blockers implemented; transaction-log replay/deleted value testimony, full SAM/SECURITY decoding, and full SRUM/EDB table decoding remain later.
- Add browser unified view across Chrome, Edge, Firefox, downloads, history, and typed URLs. Status: Chrome/Edge/Brave/Firefox profile collection exists for history/download rows, with normalized web activity pivots, dedicated AI-service usage rows, and browser-storage AI conversation candidates for common assistant/search services. Question/answer candidates are now paired into transcript pivots with completeness score, orphan counts, source hashes, and validation status for review.
