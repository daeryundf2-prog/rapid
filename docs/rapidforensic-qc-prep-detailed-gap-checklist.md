...
| 86 | Chain of custody | Capture acquisition metadata, transfers, write-blocker info, analyst actions, and export events. Current implementation lets `final-qc-report` hash `--chain-of-custody` evidence and emits `chain-of-custody-qc-contract-v1` inside `legal-submission-qc-contract-v1`. | Chain-of-custody report section with hashes. | Real operator records required. |
| 87 | Audit hash chain | Maintain append-only audit rows and export tamper-evident bundle. Current implementation lets `final-qc-report` hash `--audit-bundle` evidence and emits `audit-hash-chain-qc-contract-v1`; reviewer bundles/Case DB exports already provide audit hash-chain/tamper bundle artifacts. | Audit chain verification fixture. | External signing remains separate. |
| 88 | Court exhibit bundle | Package selected evidence, report, manifests, hashes, provenance, limitations, and signing slot. Current implementation lets `final-qc-report` hash `--exhibit-bundle` evidence and emits `court-exhibit-bundle-qc-contract-v1`; reviewer bundles already produce court exhibit manifests. | Exhibit ZIP fixture and manifest verifier. | Signed exhibit package requires external signing. |
| 89 | QC checklist | Generate pass/fail/blocked checklist from run outputs, validation package, trusted diffs, warnings, and blockers. Current implementation extends `final-qc-checklist-v1` with custody/audit/exhibit attachment checks plus validation/performance/browser/signoff checks. | QC checklist fixture with blocked and passed rows. | Final QC depends on real evidence. |
| 90 | Final QC report | Produce final QC summary with pass/fail/blocked/needs-external-evidence per feature and parser family. Current implementation expands `final-qc-execution-report-v1` to cover #81~#90 and embeds `final-qc-summary-qc-contract-v1`. | End-to-end QC report generated from a sample case. | Real Windows 11 E01 QC remains the decisive test. |
