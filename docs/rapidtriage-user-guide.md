...
- `.reg` exports can provide computer name, timezone, ProfileList SID, admin-group hints, current control set, service configuration, mounted devices, SECURITY/LSA sensitive locations, privilege assignments, last boot/shutdown timestamps, exported group membership hints, and exported account lifecycle fields such as created time, last logon, password-last-set, disabled, and admin hints. Account lifecycle rows now include an `account_security_context` that links builtin group SID candidates to inherited privilege assignments, so an analyst can see when an account appears to inherit a high-risk privilege through Administrators-style membership.
