...
For #22~#25 validation, run `rapidtriage image-workflow-validate` after a trusted tool or vendor suite produces a reference manifest. The command compares RapidTriage E01/RAW/virtual-disk/container workflow rows against references from `ewfverify`, Sleuth Kit/`tsk_recover`, `qemu-img`, FTK Imager, X-Ways/EnCase/AXIOM, or a verified vendor export manifest. A passing diff is validation evidence for that workflow only; source hashes, tool versions, commands/procedure notes, corpus scope, and reviewer signoff are still required before report-grade or commercial-grade wording.
...
Use `rapidtriage validation-diff-runners --json` to print the #76~#85 trusted-tool runner matrix. The matrix records the NIST CFReDS/CFTT-style public corpus evidence contract plus EVTX, Registry, NTFS, ESE, execution/user-activity, Windows system IR artifacts, browser/AI artifacts, E01/RAW/virtual-disk/container image workflows, and SAM/SECURITY/SYSTEM plus execution-registry trusted-tool command templates for EvtxECmd, Hayabusa, RECmd, Registry Explorer, MFTECmd, analyzeMFT, UsnJrnl2Csv, SrumECmd, libesedb, Windows Search DB Analyzer, PECmd, JLECmd, LECmd, ShellBagsExplorer/SBECmd, RegRipper, AmcacheParser, AppCompatCacheParser, Velociraptor, Chainsaw, Autoruns, Hindsight, BrowserHistoryView, DB Browser for SQLite, service exports, libewf/ewfverify, Sleuth Kit, qemu-img, FTK Imager, and vendor export manifests. It also preflights whether those binaries are on `PATH` and lists the required validation metadata flags: source evidence hash, tool version, tool command, independent report, and corpus scope. If trusted tools live outside `PATH`, add one or more `--search-path <dir>` flags. Add `--probe-versions` only in an analyst-controlled environment; it runs detected tools with bounded version probes, captures output previews/hashes, and makes the runner matrix easier to attach as QC setup evidence.
