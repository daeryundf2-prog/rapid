...
Before relying on a large Case DB search, run `rapidtriage case-db ./case.db --case-id CASE-001 --search-index-health --json`. The `search_index_health` block compares source row counts to FTS row counts for documents, files, artifacts/indicators, and timeline events, then reports missing or orphaned index rows. `case-search` also embeds this health status in `summary.search_index_health_status`, `summary.search_index_missing_rows`, and each source plan row; if a requested source is not healthy, `partial_coverage_warning` is raised even when a small bounded fallback returns hits. If health returns `needs-rebuild`, run `rapidtriage case-db ./case.db --case-id CASE-001 --rebuild-search-indexes --json`; the command rebuilds the local FTS search indexes and emits before/after health evidence plus an audit event. This is a Mac-local repair and verification step, not a substitute for external 1M/10M benchmark or trusted-tool validation evidence.
