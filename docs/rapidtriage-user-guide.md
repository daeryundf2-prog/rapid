...
Every commercial-readiness JSON also includes `blocker_execution_package`, which converts the current blocker matrix into the next bounded internal implementation batch and the next external/trusted-evidence batch. For #1~#5, the package points EVTX work at EvtxECmd/Hayabusa trusted diffs and Registry work at RECmd/Registry Explorer trusted diffs, including the RapidTriage output hint and `cross-tool-validate` command template. Use its recommended commands and acceptance-evidence checklist to prepare the next Mac-safe work without accidentally claiming validation; the package hash proves the checklist content, not commercial readiness.
