...
For image/container preflight, JSON output now includes source integrity, missing tools, external tool path/version preflight where applicable, safety notes, limitations, fallback guidance, and a `commercial_grade_ready` flag. Large images may defer full SHA256 hashing in preflight so the UI does not freeze; preserve full acquisition hashes from the imaging workflow and compare them in the case record. For E01/Ex01 report-grade preparation, `e01-known-answer` and `e01-smoke` also emit an `e01-ex01-report-grade-validation-plan-v1` object that lists the exact `e01-hash`, `ewfverify`, preflight, smoke, and trusted-diff commands plus the evidence slots still blocking report use. RAW/split image preflight and extraction metadata now emit `raw-split-report-grade-validation-plan-v1` with per-part hash, split gap/order, `mmls`, `fsstat`, `tsk_recover`, and trusted recovery-diff evidence slots. Virtual disk preflight and extraction metadata now emit `virtual-disk-report-grade-validation-plan-v1` with source hash, qemu-img info, raw conversion, converted-RAW hash, chain-risk review, nested `mmls`/`fsstat`/`tsk_recover`, and trusted conversion/recovery-diff evidence slots. Proprietary forensic-container preflight now emits `forensic-container-report-grade-validation-plan-v1` with source-container hashing, evidence preflight, trusted vendor export, sidecar manifest authoring, derived export scan, `image-workflow-validate --item-number 25`, export-root/file-hash inventory, and native/deleted/encrypted-container blockers.
...
For #22~#25 validation, run `rapidtriage image-workflow-validate` after a trusted tool or vendor suite produces a reference manifest. The command compares RapidTriage E01/RAW/virtual-disk/container workflow rows against references from `ewfverify`, Sleuth Kit/`tsk_recover`, `qemu-img`, FTK Imager, X-Ways/EnCase/AXIOM, or a verified vendor export manifest. For #26~#30 mobile/app validation, run `rapidtriage cross-tool-validate` against Cellebrite/XRY/GrayKey/AXIOM/iLEAPP/ALEAPP/MVT/apktool-normalized exports; the report includes `mobile_export_field_comparison` and `mobile_app_field_comparison` so key overlap cannot hide message/contact/iOS-backup/keychain/Android-app field mismatches. RapidTriage JSON with nested `details.messages`, `details.ios_backup_files`, `details.keychain_rows`, or `details.app_data_rows` is expanded automatically, so analysts do not need to hand-flatten common mobile export/app-data structures before trusted diffs. For #31~#40 messenger, email, provider export, and cloud API validation, the same command now emits `chat_app_field_comparison`, `email_field_comparison`, `cloud_export_field_comparison`, and `cloud_api_field_comparison` against service exports, libpff/readpst/Outlook rows, Takeout/iCloud/Purview/eDiscovery rows, or provider API spot-checks. Nested messenger `details.messages`, mailbox `details.email_messages`/`details.mailbox_messages`, cloud `details.files`/`details.events`, and API `details.responses`/`details.pages` are expanded with family guards, so generated RapidTriage outputs can be validated directly without counting the same child rows in multiple domains. A passing diff is validation evidence for that workflow only; source hashes, tool versions, commands/procedure notes, corpus scope, and reviewer signoff are still required before report-grade or commercial-grade wording.
...
rapidtriage case-search ./case.db --case-id CASE-001 -k password --limit 100 --cursor <next_cursor>
```

Each Case DB search response includes `review_workflow_summary`, which shows #51 review status counts, verification counts, assignee/priority counts, a bounded review queue, report-candidate counts, active review filters, and the remaining role-based queue/notification/multi-user blockers. It also includes `large_case_search_plan`, which records whether each requested source used SQLite FTS5 or a bounded scan, the active scan cap, and a partial-coverage warning when a large table extends beyond the responsive scan window. File path, extension, and hash searches use `file_record_fts` when the Case DB has indexed file rows, artifact/indicator title-summary-metadata searches use maintained `artifact_fts` rows, and timeline event type/time/target/description/source searches use `event_fts` when event rows are indexed, reducing the chance that a late-row filename, artifact, or event is missed by the responsive scan window. Large result sets can be paged with `summary.next_cursor`; the CLI accepts it with `--cursor`, and the web Case DB panel shows a `Next results` control when more rows are available. The cursor is bound to the case id, keywords, source filters, metadata filters, and review filters so it cannot be accidentally replayed against a different query. Each page also emits `case_search_result_window_manifest` with a stable query hash, page-window hash, per-hit row hashes, source-viewer locators, and the linked review assignment manifest hash, so a reviewer can prove which page of hits was opened before citing it. Imported cloud, email, and AI artifact hits now preserve their parser-native `source_viewer_locator`, source locator hash, row citation hash, and parser manifest hashes in the Case DB `source_reference`; review queues and report citations carry that native locator as an upstream source-viewer target so the analyst can jump back to the exact provider-export or mailbox row before citing it. If document text extraction failed during Case DB import, `summary.document_error_count` and `documents.errors[]` are returned with the search result so a no-hit document search is not mistaken for complete coverage.

Before relying on a large Case DB search, run `rapidtriage case-db ./case.db --case-id CASE-001 --search-index-health --json`. The `search_index_health` block compares source row counts to FTS row counts for documents, files, artifacts/indicators, and timeline events, then reports missing or orphaned index rows. `case-search` also embeds this health status in `summary.search_index_health_status`, `summary.search_index_missing_rows`, each source plan row, and `reportability_decision`; if a requested source is not healthy, `partial_coverage_warning` is raised and `ready_for_absence_claim` stays false even when a small bounded fallback returns hits. If health returns `needs-rebuild`, run `rapidtriage case-db ./case.db --case-id CASE-001 --rebuild-search-indexes --json`; the command rebuilds the local FTS search indexes and emits before/after health evidence plus an audit event. This is a Mac-local repair and verification step, not a substitute for external 1M/10M benchmark or trusted-tool validation evidence.

For commercial readiness triage, `rapidtriage commercial-readiness --json --limit N` now compacts high-volume stdout arrays and emits `stdout_limit_profile`. Use `--output-dir` when you need the complete JSON/Markdown evidence report; the files written there remain untrimmed.
...
Source viewer path resolution accepts absolute paths, run-root relative paths, Windows-style `C:\Users\...` paths, and bounded ZIP member locators such as `Users/alice/Documents/ChatGPT-export.zip::conversations.json`. ZIP member previews and current-entry searches are read in-memory without extraction, preserve entry CRC32/hashes and the original archive path, and keep the download/hash actions pointed at the authoritative ZIP container. For E01/Ex01 extraction trees this lets locator links map Windows artifact paths back to files under the extracted analysis root when the matching relative path exists.
...
For SQLite-backed evidence such as browser history, messenger databases, SRUM/Windows.edb exports, or app caches, `source-read` can now open a bounded read-only table locator directly from the completed run root: `rapidtriage source-read ./case-run --path Users/alice/AppData/Local/History --sqlite-table urls --sqlite-where-column url --sqlite-where-contains example --hash --json`. It can also open a single ZIP member without extraction, for example `rapidtriage source-read ./case-run --path Users/alice/Documents/ChatGPT-export.zip::conversations.json --hash --json`, preserving the archive path, entry name, CRC32, entry hashes, and zip-entry locator. Use `rapidtriage source-search ./case-run --path Users/alice/Documents/ChatGPT-export.zip::conversations.json -k evtx --json` when you need reproducible current-file or current-ZIP-entry keyword hit context from the terminal before opening the same source in the web viewer; the same command also scans SQLite `.db`/`.sqlite`/`.sqlite3` text columns in read-only bounded mode and returns table, rowid, column, offset, and snippet citations. Analysts can raise or lower the CLI SQLite scan budget with `--sqlite-row-scan-limit` to keep very large app/browser databases responsive while making the scan cap explicit in JSON. For large text/log/JSON/CSV-like sources that are too large for inline preview, `source-search` now scans a bounded byte window without loading the full file: `rapidtriage source-search ./case-run --path Users/alice/Logs/application.log -k evtx --byte-offset 2000000 --max-search-bytes 2000000 --json`. The JSON summary returns `plain_text_stream_search`, `byte_offset`, `bytes_read`, `next_byte_offset`, and `stream_truncated` so the analyst can continue the next window reproducibly. The payload includes row hashes or entry hashes, table/offset/limit or archive-entry metadata, a stable locator manifest where applicable, and clear warnings that deleted-row/WAL replay, archive completeness, and trusted schema/export diffs are still required before report-grade use. Every `source-read` payload also includes `source_citation_package`, which provides copy-safe citation text, source locator, optional source SHA256, snippet hash, package hash, and a `Current-file hit:` review-note template so verified viewer hits can be carried into review marks or report candidates without losing provenance.
...
rapidtriage case-review ./case.db --case-id CASE-001 --target-type artifact --target-id 3 --status relevant --verification-status source_opened --source-read-json ./rapidtriage-source-read.json --include-in-report
```

Every Case DB review update is versioned. `--source-read-json` appends the source viewer’s copy-safe citation text, locator snippet, and citation package hash into the review note, and it now stores the full source citation package as structured Case DB review metadata. `case-db-report` carries that package forward as `source_citation_package`, `source_citation_package_hash`, `source_review_handoff`, enriched source hashes, snippet hashes, and source locator fields, so the report handoff preserves what the analyst opened before marking the item instead of depending on free-text notes. For imported cloud, email, and AI artifact rows, the report item provenance and `citation_index` also keep the parser-native upstream `source_viewer_locator`, source locator hash, row citation hash, and parser manifest hashes that came from the artifact parser. `case-db-report` also includes both a #64 `citation_index` and each item’s #65 `review_history`, with #64/#65 `core_accuracy_gates`, so a reviewer can see when an item became report-worthy, who changed it, and which source citation the decision refers to. The #65 export also includes per-row history hashes and an export-time history head hash so the final report bundle can preserve a tamper-evident selection snapshot, while still warning that signed multi-user append-only history is not yet implemented.
...
rapidtriage commercial-readiness --mac-first-evidence ./qc --output-dir ./commercial-readiness --json
```
...
`email-external-parse` is the Mac-first bridge for PST/OST/MSG evidence when native full MAPI decoding is not yet report-grade. It runs `pffexport`, `readpst`, `msg-extractor`, or an explicit `--preferred-tool` when available, inventories exported files, and writes `email-external-parser.json` plus a Markdown report. The JSON records the source hash, tool availability matrix, selected tool path, command argv hash, stdout/stderr hashes, export inventory hash, validation checks, an `email-external-parser-evidence-manifest-v1` hash, and an `email-external-export-review-profile-v1`. The review profile parses exported EML/MBOX candidates within bounded caps, summarizes message headers/body previews/attachment counts/folder candidates, and emits source-viewer locators and row hashes so analysts can review extracted mail without claiming native PST/OST decoding. If the tool is missing or the parser fails, the command still writes blocker evidence instead of pretending the mailbox was decoded.

`large-case-readiness` is the Mac-first gate for large evidence sets. It combines one or more `sqlite-fts-benchmark` JSON files with an optional RapidTriage Case DB profile, then records whether 100k/1M/10M search evidence, FTS table/index metadata, p95 latency thresholds, and remaining commercial blockers are present. When a Case DB is attached, the report emits `case-db-search-diagnostics-v1`, `case-db-cursor-diagnostics-v1`, and `case-db-search-index-health-summary-v1`: per-FTS-table row counts, keyword `MATCH` counts, `EXPLAIN QUERY PLAN` details, deterministic cursor page-window hashes, next-offset hashes, missing/orphan search-index rows by case, stable hashes, and readiness blockers. If `summary.case_db_search_index_healthy` is false, rebuild indexes before relying on no-hit or absence claims. If `summary.case_db_cursor_pagination_proven_tables` is zero, seed or attach a Case DB with at least two matching rows before claiming pagination has been exercised. A failed status does not mean the tool cannot search; it means the evidence is not strong enough yet for 1TB/10TB or commercial-grade claims.

`macos-live-smoke` is designed for the analyst's current Mac. It writes a macOS collect-plan summary, redacted live artifact counts, a small triage benchmark, SQLite FTS benchmark evidence, a `large-case-readiness.json` gate built from that benchmark, validation-tool availability, and an executable `attach-mac-evidence-to-readiness.sh` helper that reruns `commercial-readiness` with the smoke evidence attached. If you already have a RapidTriage Case DB, pass `--case-db ./case.db` so the smoke report also profiles FTS tables, indexes, table counts, and Case DB search readiness. By default it stores path hashes and counts only; it does not print browser history URLs, quarantine URLs, TCC client paths, or the Case DB path. Use `--include-path-details` only for authorized local debugging.

Use `commercial-readiness --mac-first-evidence ./qc` after a Mac hardening run to auto-discover supported evidence under a QC folder, or pass individual files such as `./qc/macos-live/macos-live-smoke.json`, `./qc/source-read.json`, `./qc/source-search.json`, `./qc/email-external/email-external-parser.json`, and `./qc/cloud-export/cloud-export-artifacts.json`. Source viewer/search evidence records citation package hashes, review-note readiness, source locator type, match counts, and court-report blockers for #52/#61/#64/#65 handoff checks. The email external evidence shows PST/OST/MSG external-parser evidence, export inventory hashes, exported-message review counts, trusted-diff readiness, and the affected readiness items (#36, #55, #81, #85, #90, #95). Cloud export evidence shows Google Takeout/Gmail, iCloud, M365/Teams/OneDrive, and AI service export parser output counts, service pivots, parser/archive manifest hashes, Q/A pair counts, orphan question/answer counts, minimum transcript completeness, provider reportability blockers, required-before-report steps, and provider-diff blockers for #21/#37/#38/#39. Image workflow evidence can now be attached as `image-workflow-validate` JSON for #22/#23/#24/#25, showing matched source/container/partition/export fields against a trusted image workflow reference. The smoke JSON includes a `readiness_attachment` block with the exact CLI command, API query hint, GUI note, supported Mac-preparable backlog item numbers, and required follow-up evidence. The `mac_first_evidence_summary` hashes the attached JSON and lists failed checks/blockers, but it is deliberately not treated as a passing validation package; Windows image tests, 1TB-10TB hardware runs, signed installers, trusted mailbox/provider parser diffs, and independent signoff stay as separate blockers.

Every commercial-readiness JSON also includes `blocker_execution_package`, which converts the current blocker matrix into the next bounded internal implementation batch and the next external/trusted-evidence batch. For #1~#30, the package can point EVTX work at EvtxECmd/Hayabusa trusted diffs, Registry work at RECmd/Registry Explorer trusted diffs, SAM/SECURITY/SYSTEM and Amcache/ShimCache/BAM/DAM work at RECmd/RegRipper/AmcacheParser/AppCompatCacheParser trusted diffs, SRUM/Windows.edb work at SrumECmd/libesedb/Windows Search DB Analyzer trusted diffs, MFT/USN work at MFTECmd/analyzeMFT/UsnJrnl2Csv trusted diffs, JumpList/ShellBags work at JLECmd/ShellBagsExplorer/SBECmd trusted diffs, Prefetch work at PECmd trusted diffs, LNK work at LECmd/JLECmd trusted diffs, Windows system artifact work at Velociraptor/Chainsaw/Autoruns trusted diffs, browser storage/timeline/AI transcript work at Hindsight/BrowserHistoryView/DB Browser/Velociraptor/service-export trusted diffs, E01/RAW/virtual-disk/container work at `image-workflow-validate` with libewf/Sleuth Kit/qemu-img/vendor export references, and mobile/iOS/Android/APK work at Cellebrite/XRY/GrayKey/AXIOM/iLEAPP/ALEAPP/MVT/apktool trusted diffs. Each hint includes the RapidTriage output hint and `cross-tool-validate` or `image-workflow-validate` command template. Use its recommended commands and acceptance-evidence checklist to prepare the next Mac-safe work without accidentally claiming validation; the package hash proves the checklist content, not commercial readiness.

In the web dashboard, open the commercial-readiness panel and paste the same QC folder path into `Mac evidence/QC folder`, then click `Apply Mac evidence`. The panel stores that local path in the browser only, reruns `/api/commercial-readiness?mac_first_evidence=<path>`, and shows each attached Mac evidence row separately from validation packages. Use this as a reviewer checklist: smoke score, large-case status, source viewer handoff readiness, source-search hit counts, email export counts, cloud/AI export parser counts, trusted-diff readiness, evidence hashes, and failed checks should be visible before you decide what still needs Windows, hardware, or independent validation.
...
- Browser rows normalize Chrome/Edge/Brave/Firefox history and downloads into profile-scoped history/download lists, internet usage pivots, AI service visit rows, bounded unified timeline rows, #20 `forensic_review`, and `browser_analyst_review_profile`. Chromium and Firefox are supported on Windows and macOS through local profile databases; Safari history is supported on macOS, and Safari downloads are correlated from LaunchServices `QuarantineEventsV2` when present. Treat Safari cache/session/deleted-state parity as validation-gated.
...
- Email and cloud rows use the same review-card model. `email_analyst_review_profile`, `cloud_analyst_review_profile`, and `cloud_api_analyst_review_profile` are designed for the right-side preview/detail pane: they show the source hash, citation manifest, primary pivots, trusted-parser/provider diff targets, legal/privilege/scope questions, and clear non-claims before the analyst marks the row as evidence. Email rows also include `email_report_grade_validation_plan`, which separates ready pivots such as source hash, row citation, thread/participant profile, bounded MAPI inventory boundary, source viewer locator, hash-only body policy, and attachment locator from blockers such as trusted mailbox diff, native MAPI/deleted-item validation, auth/crypto validation, privilege review, and independent signoff.
...
- Event rows include `event_message` and `message_rendering` provenance. Exported/rendered messages are preserved when present; native EVTX rows can use built-in high-value fallback templates for common Security, PowerShell, and log-clear events. The collector also auto-discovers bounded case-local provider catalogs: `.man` files, plus JSON/XML files with message-catalog/provider-manifest naming hints. Use `--eventlog-message-catalog` when you need to supplement or override discovered entries with a curated JSON catalog or Windows Event Manifest XML (`.man`/`.xml`) string table. Catalog templates support named `{Field}` placeholders and Windows-style positional `%1`, `%2` placeholders using ordered EventData/TemplateValue values, and manifest `valueMap`/`bitMap` labels are applied with raw values preserved in `used_fields`. Native fallback messages are marked validation-required until provider message resources or trusted-tool rendered-message diffs are resolved.
- Slack/deleted/corrupt EVTX recovery is cautious by design. Parseable rows can be labeled `slack-or-deleted-record-candidate` when chunk free-space metadata supports that interpretation, and invalid record headers emit `eventlog-record-candidate` rows with offsets, size checks, confidence, and `do-not-report-without-validation` caution labels. Both parseable slack/deleted rows and corrupt candidates can carry `evtx_recovery_report_citation_manifest`, which preserves source hash, record offset/size/hash, chunk/free-space context, candidate class, required independent checks, reportability blockers, and a stable manifest hash for reviewer handoff without treating recovered records as final proof.
...
- `windows-execution` imports Amcache/ShimCache/UserAssist/BAM-style `.reg` exports, scans native `Amcache.hve` and bounded native SYSTEM-hive ShimCache/BAM/DAM string pivots, and imports PowerShell console history. Amcache, ShimCache, BAM/DAM, and SRUM rows expose #7~#10 `core_accuracy_gates` showing which required validation checks are satisfied and which commercial-grade blockers remain. Amcache rows additionally emit `amcache_schema_version_profile` and a stable `amcache_row_manifest_hash` so GUI/report workflows can cite the schema hint, source locator, normalized path/hash identity, timestamp semantics, required trusted-diff fields, and reportability blockers. ShimCache rows additionally emit `shimcache_layout_profile`, `shimcache_row_manifest_hash`, source-viewer locators, cache-order/source-offset identity, OS-build layout blockers, and a fixed not-proof-of-execution caveat, so reviewers can pivot on program-presence/order evidence without calling it execution proof. BAM/DAM rows additionally emit `bam_dam_row_manifest_hash`, ControlSet/source-key/source-offset identity, SID/device path/timestamp semantics, and correlation-before-testimony blockers so recent-execution pivots stay source-verifiable. SRUM rows additionally emit `srum_report_citation_manifest_hash`, source-format/counter/timestamp semantics, network profile/interface pivots, and mandatory SRUM caveat coverage so usage rows and native candidates stay source-verifiable. They also emit `execution_analyst_review_profile` so the reviewer sees severity, "not proof of" caveats, primary pivots, populated source values, and the exact artifacts to correlate before reporting.
...
- SRUM CSV/JSON/JSONL/NDJSON exports from trusted tools can be imported as app resource or network usage rows with bytes totals, interface/profile, energy, user, timestamp, source hashes, and stable citation manifest hashes. `SRUDB.dat` is also preserved with ESE header/page-size and native validation metadata, bounded string/path/URL pivots, native `srum-database-pivot` rows, `srum-table-candidate` rows, and bounded `srum-row-candidate` string-cluster rows for app/network/energy/user review. SRUM manifests preserve source format, table/app identity, counter/timestamp semantics, network profile/interface pivots, source offset or row-cluster locator, and a semantics warning. Treat native SRUDB candidates as validation-required until a dedicated SRUM/ESE parser confirms tables, counters, timestamps, page checksums, and tagged-column rows.
- `windows-search-index` inventories `Windows.edb` with ESE header metadata, bounded string/path/URL/content pivots, native validation metadata, explicit commercial-readiness blockers, separate `windows-search-edb-pivot`, `windows-search-edb-page-candidate`, and `windows-search-edb-table-candidate` rows, and correlated `windows-search-edb-row-candidate` rows that pair path/URL/content strings for review while clearly marking timestamp/deleted-state gaps. Page candidates preserve ESE page index, byte offset, page SHA256, page-local table marker hits, and page-local path/URL/content/risk strings so reviewers can jump back to a stable source location. Rows now include `windows_edb_report_citation_manifest_hash`, `edb_semantics_warning`, `windows_edb_trusted_diff_contract`, and `windows_search_analyst_review_profile` so the UI can show not-proof-of warnings, stable citation hashes, trusted-diff requirements, source pivots, and MFT/USN/browser/document correlation targets beside each hit. It also imports Windows Search CSV/JSON exports so indexed filenames, paths, URLs, titles, and content snippets can be searched alongside documents and artifacts.
...
- Mobile/vendor KakaoTalk rows now include `kakaotalk_report_grade_validation_plan`. The plan shows what is internally ready for review, such as source hashing, row citation, message-pivot normalization, BigBang compatibility classification, hash-only text policy, and source-viewer locator, and what still blocks report-grade testimony, such as trusted KakaoTalk export/native DB diff, post-BigBang known-answer corpus, schema-version map, encrypted-store authority, deleted/read-state validation, attachment byte validation, and independent review. Treat the row as a searchable/reviewable KakaoTalk pivot until those blocking slots are attached.
- Mobile/vendor WhatsApp rows now include `whatsapp_report_grade_validation_plan`. The plan separates ready review pivots, such as source export/msgstore integrity, row citation, message/JID/media normalization, msgstore inventory boundary, crypt/export strategy classification, hash-only text policy, and source-viewer locator, from blockers such as trusted WhatsApp export/native DB diff, lawful crypt-key authority workflow, msgstore/wa.db schema corpus, deleted-row corpus, contact/call/media recovery validation, timezone/ack/read-state semantics, attachment byte validation, and independent review. Treat WhatsApp rows as authorized export/msgstore triage until those slots are satisfied.
- Mobile/vendor Telegram rows now include `telegram_report_grade_validation_plan`. The plan separates ready review pivots, such as source export/cache/database integrity, row citation, message/account/dialog/media normalization, DB/cache inventory boundary, export/cache strategy classification, hash-only text policy, and source-viewer locator, from blockers such as trusted Telegram export/native DB diff, lawful local-store decryption authority workflow, export schema-version corpus, account/dialog/peer attribution corpus, secret/edited/deleted semantics validation, cache media locality validation, and independent review. Treat Telegram rows as authorized export/cache triage until those slots are satisfied.
- Mobile/vendor Signal rows now include `signal_report_grade_validation_plan`. The plan separates ready review pivots, such as source export/SQLCipher integrity, row citation, thread/recipient/message/attachment normalization, Signal DB/SQLCipher inventory boundary, SQLCipher strategy classification, hash-only text policy, and source-viewer locator, from blockers such as trusted Signal export/native DB diff, lawful SQLCipher key authority workflow, recipient/thread schema corpus, attachment locality validation, deleted/disappearing message semantics, delivery/read-state semantics, and independent review. Treat Signal rows as authorized export/SQLCipher-inventory triage until those slots are satisfied.
- Mobile/vendor extended messenger rows now include `extended_messenger_report_grade_validation_plan`. The plan separates ready review pivots, such as source export/native row integrity, service row citation, message/media/reaction normalization, service source-track classification, hash-only text policy, and source-viewer locator, from blockers such as trusted extended messenger export/native DB diff, per-service schema-version corpus, encrypted/ephemeral authority workflow, media locality validation, reaction/read/edit/delete semantics, service coverage matrix, and independent review. Treat WeChat/LINE/Discord/Instagram-style rows as service export/native-inventory triage until those slots are satisfied.
...
- Bounded ChatGPT/Claude/Gemini/Perplexity-style JSON/JSONL service exports under the evidence root, including JSON/JSONL entries inside service export ZIP archives and `Takeout.zip`-style archives, emitted as `ai-service-export-conversation` rows with Q/A pairing, source hashes, source offsets, JSON Pointer locators, row citation hashes, text hashes, service export schema profiles, archive entry context when applicable, service-export parser manifests, and source-viewer citation metadata. Provider scope/schema validation, archive completeness, and trusted export diffs are still required before report-grade AI transcript claims. For #21 validation handoff, `cross-tool-validate --backlog-item 21` now emits `ai_transcript_field_comparison` so a service export or Hindsight/DB Browser reference must match the RapidTriage service, conversation/pair ID, question, answer, source hash, source path/offset, and pairing-confidence fields before the comparison can pass.
...
When a mobile acquisition/export folder contains `.apk` files, RapidTriage can inventory them without performing phone acquisition itself. The APK collector records file hashes, package/version metadata when the manifest is readable, requested permissions, dangerous permissions, component summaries, dex/native-library counts, entry hashes, native architectures, certificate entries, bounded dex/native string pivots, URL/IP indicators, validation checks, #30 `forensic_review`, and risk flags. APK rows include `android_apk_report_grade_validation_plan`, which fixes the report handoff around source APK hashing, ZIP entry inventory, manifest/package context, permission/component normalization, DEX/native pivot boundaries, signing-entry inventory, source viewer locator, trusted APK tool diff, binary-manifest validation, signature-chain/lineage validation, malware-behavior known-answer corpus, and independent signoff. It also inventories Android app-data export files under common `Android/data`, `Android/media`, and `data/data` paths by inferred package/category with #29/#30 `forensic_review` and without decoding secrets. App-data rows include `android_app_data_report_grade_validation_plan`, which fixes the report handoff around source hashing, package/path attribution, read-only SQLite schema inventory, artifact-family hints, redaction proof, source viewer locator, acquisition-manifest attribution, trusted ALEAPP/vendor diff, app-schema fixtures, encrypted/deleted-store corpus, and independent signoff.
...
The collector records source hashes, source tool hints, row indexes, source record IDs, normalized timestamps, participants, message text and text hash, contact/call fields, app package/version fields, account identifier hashes, media/browser pivots, file hashes/paths, validation checks, #26 `forensic_review`, and risk flags such as OTP/password text, AI-service app/conversation references, privacy/evasion apps, and structured data files. Source-summary rows include `mobile_vendor_export_validation_plan`, which tells an analyst exactly which report-use evidence is complete and which remains blocked: source export hash, detected vendor family, sidecar linkage, vendor version/settings, original acquisition hash, schema mapper/row identity, trusted vendor diff, per-version fixtures, deleted-record corpus, and independent signoff. iOS backup source rows also include `ios_backup_report_grade_validation_plan`, which binds Manifest.db hash/read-only table evidence, required backup plist/root checks, keychain linkage inventory, app database candidate mapping, encrypted-backup authority review, trusted parser diff, app-schema known answers, deleted-record corpus, and independent signoff slots into a hashable #27 report-use checklist. iOS keychain rows include `ios_keychain_report_grade_validation_plan`, which binds keychain DB hash, read-only table inventory, redaction proof, table-class/sensitive-column map, source viewer locator, controlled-reveal audit boundary, trusted keychain diff, protected-data/keybag corpus, access-group semantics corpus, and independent signoff slots into a #28 report-use checklist without exposing secrets. KakaoTalk, WhatsApp, Telegram, Signal, WeChat, LINE, Discord, and Instagram rows from authorized exports are service-labeled with chat/message IDs, conversation titles, participants, media references, reactions, validation blockers, service-specific #31~#35 gap mapping, and `chat_app_forensic_review`. KakaoTalk rows additionally expose `kakaotalk_compatibility_assessment`; 25.7.2 / 2025-08-13 and later is treated as post-BigBang and not compatible with legacy decoding assumptions until a known-answer corpus proves otherwise. App SQLite/DB candidates are inventory-only: table names, columns, row counts, message-table candidates, hashes, and warnings are recorded, but encrypted stores and deleted records are not decoded. Each mobile export source also emits a correlation summary with message/media/contact/call counts, detected services, participants, schema-version candidates, message-media links, unified contact/call/SMS actor pivots, #43~#45 `forensic_review`, and correlation-readiness checks. iOS backup/keychain rows include #27/#28 `forensic_review`; keychain handling is inventory/redaction-only and records table names, columns, row counts, and hashes, but does not decrypt or expose passwords, tokens, cookies, or protected values. It imports vendor exports; it does not decrypt proprietary acquisition packages or replace the original mobile forensic tool validation.
...
RapidTriage can normalize cloud exports that were already lawfully exported or collected by another workflow. For Google Takeout-style folders, it recognizes Location History `Records.json` and My Activity JSON. For Apple/general account exports, it records account profile fields from JSON. ZIP provider exports are inventoried without extraction as `cloud-export-archive` rows so the original archive SHA256, bounded entry manifest, provider/product counts, JSON/CSV entry counts, and archive-scope blockers can be reviewed before parsing individual rows. Small Gmail Takeout `.mbox` entries plus bounded JSON and CSV entries inside ZIP exports are also parsed in-place and emitted as searchable `cloud-mail`, `cloud-file`, `cloud-location`, `cloud-activity`, `cloud-message`, or `cloud-audit` rows with archive entry name, CRC32, entry index, row/message index, and source archive hash. ChatGPT/Claude/Gemini/Perplexity-style JSON/JSONL service exports placed in the same cloud export folder, including bounded entries inside service export ZIP archives, emit `ai-service-export-conversation` rows with Q/A pairing, archive entry context, source hashes, and service-export validation blockers. Standalone CSV exports such as M365/Purview audit logs are read with a row cap and emitted with `source_format=csv` plus `csv_row_index` for citation.

```bash
rapidtriage artifacts ./cloud-export --kind cloud-export --output cloud-artifacts.json
```

The collector records source hashes, timestamps, activity titles/products, account profile fields, location coordinates, accuracy, and risk flags such as precise location or user activity. It also normalizes Gmail/Drive-style mail and file JSON, Apple/iCloud account/file/photo-style JSON, Microsoft 365/OneDrive/Teams/Audit JSON or CSV, collaboration-SaaS message/file-style JSON, bounded JSON/CSV-in-ZIP rows, bounded service-export AI transcript rows, and bounded Gmail Takeout MBOX-in-ZIP messages into mail, file, location, activity, chat message, audit, and AI conversation pivots with parser manifests, source-viewer locators, and validation blockers. Google rows include `google_takeout_report_grade_validation_plan`, which tells the reviewer exactly which source hash, row citation, product pivot, thread/body, sidecar/timezone, provider diff, archive completeness, schema, and independent-review slots are complete or still blocking report-grade use. Apple/iCloud rows similarly include `icloud_export_report_grade_validation_plan` for source hash, row citation, account/file/photo/mail pivots, Photos EXIF/album/share sidecars, ADP/container scope, provider diff, device association, schema, and independent-review blockers. Microsoft 365 rows include `m365_export_report_grade_validation_plan` for Teams thread/attachment/reaction slots, OneDrive/SharePoint permission and retention/version/deleted slots, audit-operation pivots, Purview/eDiscovery scope, Graph pagination, provider diff, schema, and independent-review blockers. Slack/Discord/Dropbox/Box/Zoom/Notion/Atlassian/GitHub-style message exports now add `collaboration_message_review_profile`, so reviewers can pivot by workspace, channel, thread, message ID, attachment inventory, reaction summary, and edited/deleted hints without expanding raw JSON first. ZIP archive rows include `cloud_archive_manifest`, `cloud_archive_manifest_hash`, product counts such as Gmail/Drive/Location/Teams/OneDrive/iCloud, suffix counts, and `core_accuracy_gates` evidence refs for GUI/report citation. It does not perform unrestricted live cloud acquisition, provider-side deletion recovery, permission graph reconstruction, split-archive completeness proof, large-MBOX/full-archive pagination beyond bounded triage caps, provider AI export completeness proof, workspace-wide collaboration export completeness proof, or eDiscovery validation.
...
Use `rapidtriage validation-diff-runners --json` to print the #76~#86 trusted-tool runner matrix. The matrix records the NIST CFReDS/CFTT-style public corpus evidence contract plus EVTX, Registry, NTFS, ESE, execution/user-activity, Windows system IR artifacts, browser/AI artifacts, E01/RAW/virtual-disk/container image workflows, mobile/iOS/Android/APK exports, and SAM/SECURITY/SYSTEM plus execution-registry trusted-tool command templates for EvtxECmd, Hayabusa, RECmd, Registry Explorer, MFTECmd, analyzeMFT, UsnJrnl2Csv, SrumECmd, libesedb, Windows Search DB Analyzer, PECmd, JLECmd, LECmd, ShellBagsExplorer/SBECmd, RegRipper, AmcacheParser, AppCompatCacheParser, Velociraptor, Chainsaw, Autoruns, Hindsight, BrowserHistoryView, DB Browser for SQLite, service exports, libewf/ewfverify, Sleuth Kit, qemu-img, FTK Imager, vendor export manifests, Cellebrite/XRY/GrayKey/AXIOM, iLEAPP, ALEAPP, MVT, and apktool/aapt/jadx. It also preflights whether those binaries are on `PATH` and lists the required validation metadata flags: source evidence hash, tool version, tool command, independent report, and corpus scope. If trusted tools live outside `PATH`, add one or more `--search-path <dir>` flags. Add `--probe-versions` only in an analyst-controlled environment; it runs detected tools with bounded version probes, captures output previews/hashes, and makes the runner matrix easier to attach as QC setup evidence.
...
For trusted-tool validation, use `rapidtriage cross-tool-validate --rapid-output ./case-run/artifacts/eventlog.json --reference-output evtxecmd=./EvtxECmd.csv --source-evidence ./Security.evtx --tool-version evtxecmd="EvtxECmd 1.5.0" --tool-command evtxecmd="EvtxECmd.exe -f Security.evtx --csv out" --independent-report ./independent-review.md --corpus-scope "NIST Security.evtx plus local corrupt-record fixture" --backlog-item 1 --backlog-item 2 --output ./evtx-cross-tool.json --json`. When the comparison passes and the output JSON exists, that report can be supplied directly as `commercial-readiness --validation-package ./evtx-cross-tool.json`; the embedded dataset maps the evidence to the selected backlog items. The report preserves SHA256/size/mtime for source, independent review, RapidTriage output, and reference output paths, plus tool version/command metadata and corpus scope. For EVTX-style rows it now adds `record_field_comparison`, which compares common fields such as EventRecordID, EventID, provider, channel, computer, and event time for overlapping records so a key-overlap pass cannot hide field-level parser mismatches. Use this for EVTX/Registry/SAM-account/execution-artifact/MFT/USN/JumpList/ShellBags/Prefetch/LNK/Windows-system/browser/mobile/messenger/email/cloud-style external comparisons; #6 adds `os_account_field_comparison` for SID/RID/account/group/privilege/redaction fields, #7~#9 add `execution_artifact_field_comparison` for Amcache/ShimCache/BAM/DAM path, SID, timestamp, source key/offset, hash, semantics warning, and evidence-status fields, with Amcache required-field coverage blocking commercial evidence if path, SHA1, or caveat text is absent and BAM/DAM required-field coverage blocking commercial evidence if source identity or correlation warning is absent from either RapidTriage or the trusted parser row. #14~#17 add `user_activity_field_comparison` for JumpList, ShellBags, Prefetch, and LNK AppID, target path, source path, MRU, run/access count, shell item, and tracker GUID fields, #18 adds `system_artifact_field_comparison` for Task Scheduler, Defender, Firewall, WER, and WMI task URI, command, principal, trigger, threat, network tuple, exception, and persistence-pivot fields, #19/#20 add `browser_storage_field_comparison` plus `browser_timeline_field_comparison` for browser storage type/name/path/count/size/sensitivity and unified visit/download URL, timestamp, transition, title, source table, and source index fields, #26~#30 add `mobile_export_field_comparison` plus `mobile_app_field_comparison` for mobile message/contact/call/media/iOS backup/keychain/Android app/APK package, permission, cert, and hash fields, and #31~#40 add `chat_app_field_comparison`, `email_field_comparison`, `cloud_export_field_comparison`, and `cloud_api_field_comparison` for messenger message IDs/timestamps/participants/hashes, mailbox Message-ID/folder/attachment/body hashes, provider record/item/action/hash fields, and API endpoint/status/response-hash pagination fields. USN validation also includes `usn_state_replay_field_comparison`, which expands nested `bounded_state_replay_preview.transitions` rows and compares transition, previous/new path, timestamp, file name, cursor, and state effect against known-answer replay CSV/JSON rows. The report also emits `validation_qc_contract` (`validation-qc-controls-v1`) with a trusted-diff mismatch dashboard, FP/FN recording schema, parser confidence/reportability matrix, legal limitation guardrails, and an automatic QC checklist. The report-level commercial-grade envelope can pass, but item-level commercial grade still depends on the backlog item's remaining native-parser and external-corpus blockers.

For #8 ShimCache specifically, the trusted diff must include enough field coverage to prove the row being compared still carries path identity, cache-order/source-offset identity, and the “not proof of execution” caveat. If those fields are absent from RapidTriage or the trusted AppCompatCacheParser/ShimCacheParser/RECmd-style output, the comparison remains useful for triage but is blocked from commercial evidence.
