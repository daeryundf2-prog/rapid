...
Every Case DB review update is versioned. `--source-read-json` appends the source viewer’s copy-safe citation text, locator snippet, and citation package hash into the review note so the later report export preserves what the analyst opened before marking the item. `case-db-report` includes both a #64 `citation_index` and each item’s #65 `review_history`, with #64/#65 `core_accuracy_gates`, so a reviewer can see when an item became report-worthy, who changed it, and which source citation the decision refers to. The #65 export also includes per-row history hashes and an export-time history head hash so the final report bundle can preserve a tamper-evident selection snapshot, while still warning that signed multi-user append-only history is not yet implemented.
