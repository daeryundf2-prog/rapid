...
- Native `.evtx` files are inventoried with source hashes and parser guidance; recoverable binary record headers also emit partial `eventlog-event` rows with record ID, timestamp, record SHA256, record-size integrity checks, sequence-gap hints, extracted UTF-16 strings, first-pass BinXML token scans, TemplateInstance value spec/value decoding for common scalar types, substitution value fields, promoted `Event/System`, `EventData`, and `UserData` fields, rendered previews where possible, channel/provider/computer/command/IP/user candidates, explicit `evtx_binxml_status`, and suspicious-term flags. Treat native rows as triage pivots unless an EVTX-capable export validates complete provider message rendering.
...
- Jump List files are inventoried as automatic/custom destination containers; recoverable OLE/CFB streams are listed, and embedded Shell Link records are promoted into destination rows with stream provenance, target paths, working directories, target timestamps, link flags, and offsets. When stream parsing is not possible, RapidTriage falls back to embedded path extraction for search triage.
