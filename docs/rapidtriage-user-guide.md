...
Search reaches indexed document text, EML/MBOX/MSG email text, file metadata, artifact summaries, and timeline events. Artifact results keep reviewable source paths and high-value metadata such as event IDs, usernames, source IPs, command lines, PowerShell script blocks, MFT paths, USN reasons, macOS quarantine URLs, browser history previews, and LaunchAgent labels/programs. Use source filters to narrow heavy cases.
