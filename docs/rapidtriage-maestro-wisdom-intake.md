...
- Add Windows account/OS parser: hostname, timezone, last boot, users, admin membership, created/deleted/login times. Status: hostname/timezone/profile/admin hints plus current control set, service configuration, mounted-device, LSA-sensitive-location with non-decrypting SECURITY value metadata, privilege-assignment, exported group-membership rows, exported last boot/shutdown, and account created/last-logon/password/admin/disabled hints are normalized; native SAM account-name/RID and group/alias key candidate rows are exposed with validation guidance; full SAM F/V attribute decoding, native alias-member reconstruction, and SECURITY secret decryption remain later.
- Add commercial gap item 121: deep `NTUSER.DAT` and `UsrClass.dat` user-hive analysis, including full key tree reconstruction, UserAssist, TypedURLs/TypedPaths, RecentDocs, Run/RunOnce, Explorer/MRU keys, ShellBags, MountPoints2, Network, ComDlg32/OpenSavePidlMRU, deleted key/value recovery, per-key last-write timeline, parser confidence, and fixture-backed validation. Status: first-pass `registry-user-activity` rows now promote UserAssist, TypedURLs/TypedPaths, Run/RunOnce, Explorer/MRU, ShellBags, MountPoints2, Network, and ComDlg32/OpenSavePidlMRU pivots from `.reg` exports and bounded native NTUSER/UsrClass string scans; native hives now emit hbin-aware key-tree plus key/value recovery candidates, while binary value decoding, transaction-log replay, deleted-value testimony, and report-grade user activity claims still require a dedicated registry parser or trusted export validation.
