...
| #62 Keyword packs | Built-in packs and JSON custom-pack support preserve keyword counts, provenance, deduplication, API search `keyword_pack_selection_profile`, web pack-selection controls, trusted expansion diff helper/gate, and #62 `core_accuracy_gates`. | Per-case pack editor, signed pack library, trusted expansion manifest, language/domain-specific packs, and release-reviewed pack versions. |
