...
| --- | --- | --- |
| #61 Advanced search | Search output records query mode/options, fuzzy/stem/regex behavior, proximity metadata, hit pointers, `advanced_search_profile`, regex query validation, per-hit/top-level query-hit manifests, `advanced_search_report_grade_validation_plan`, validation-plan hashes, ready/blocking slot counts, web mode/fuzzy/proximity controls, trusted query-hit diff helper/gate, and #61 `core_accuracy_gates`. | Source-row hash verification for report candidates, large multilingual relevance corpus, trusted query-hit manifest, tuned false-positive/false-negative measurements, large-case search performance validation, and full browser query-builder validation. |
| #62 Keyword packs | Built-in packs and JSON custom-pack support preserve keyword counts, provenance, deduplication, per-pack/library/selection manifests, keyword row hashes, expansion head hashes, manifest hashes, API search `keyword_pack_selection_profile`, `keyword_pack_report_grade_validation_plan`, validation-plan hashes, ready/blocking slot counts, web pack-selection controls, trusted expansion diff helper/gate, and #62 `core_accuracy_gates`. | Per-case pack editor/audit, signed/versioned pack library, release-review records, trusted expansion manifest corpus, language/domain-specific packs, large-case pack performance validation, and release-reviewed pack versions. |
| #63 IOC/TI enrichment | Indicator summaries and the API/web local enrichment review package preserve source links, local rule hits, offline feed provenance with feed hash/size, feed/indicator/enrichment manifests, indicator/feed row hashes, match mode, no-external-call warning, `ioc_ti_report_grade_validation_plan`, validation-plan hashes, ready/blocking slot counts, trusted enrichment diff helper/gate, and #63 `core_accuracy_gates`. | Signed feed packages, STIX/TAXII/provider validation, trusted enrichment manifest corpus, confidence decay, false-positive/source-correlation corpus, feed trust workflow, and external enrichment governance. |
| #64 Citation manager | Case DB report exports preserve review/source citation IDs, copy-safe citation strings, citation row hashes, source-viewer locators, source references, source-hash/parser-version coverage profile, `report-citation-index-manifest-v1`, `report_citation_report_grade_validation_plan`, validation-plan hash, ready/blocking slot counts, citation counts, report-use warnings, trusted citation-index diff helper/gate, and #64 `core_accuracy_gates`. | Exhibit numbering UI, trusted citation manifest corpus, jurisdiction templates, source-hash/parser-version completeness validation across every parser, and reviewer signoff corpus. |
...
The #61 through #65 rows now include reportability decisions and trusted diff helper/gates. Advanced search is limited to `advanced-search-triage-pivot` with a report-grade validation plan; keyword packs to `keyword-pack-expansion-triage-pivot` with a report-grade validation plan; IOC/TI enrichment to `offline-ioc-ti-triage-pivot` with a report-grade validation plan; citation exports to `report-citation-index-triage-pivot` with a report-grade validation plan; and evidence-selection history to `evidence-selection-history-triage-pivot`. These are not source-proof, release-reviewed pack, live TI verdict, court exhibit, or multi-user signed-history claims until the missing corpus, trusted manifests, signing, feed provenance, source-hash/parser-version completeness, reviewer signoff, large-case performance, and append-only evidence is attached.
