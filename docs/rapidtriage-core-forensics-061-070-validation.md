...
| #64 Citation manager | Case DB report exports preserve review/source citation IDs, copy-safe citation strings, citation row hashes, source-viewer locators, source references, source-hash/parser-version coverage profile, `report-citation-index-manifest-v1`, citation counts, report-use warnings, trusted citation-index diff helper/gate, and #64 `core_accuracy_gates`. | Exhibit numbering UI, trusted citation manifest corpus, jurisdiction templates, and source-hash completeness validation across every parser. |
...
The #61 through #65 rows now include reportability decisions and trusted diff helper/gates. Advanced search is limited to `advanced-search-triage-pivot` with a report-grade validation plan; keyword packs to `keyword-pack-expansion-triage-pivot` with a report-grade validation plan; IOC/TI enrichment to `offline-ioc-ti-triage-pivot` with a report-grade validation plan; citation exports to `report-citation-index-triage-pivot`; and evidence-selection history to `evidence-selection-history-triage-pivot`. These are not source-proof, release-reviewed pack, live TI verdict, court exhibit, or multi-user signed-history claims until the missing corpus, trusted manifests, signing, feed provenance, source-hash, reviewer, large-case performance, and append-only evidence is attached.
