...
| --- | --- | --- |
| #101-#104 Packaging/update | Release scripts generate portable ZIP, release manifest, packaging plan, update manifest, checksums, and platform-specific evidence gates. | Real signed Windows installer, notarized macOS package, deb/rpm/AppImage builds, hosted signed update channel, and fresh host smoke logs. |
| #105-#110 Enterprise operations | Enterprise policy and crash reports document local-only operation, redaction, offline license posture, RBAC policy, multi-user guardrails, and audit trail limits. | UI crash export, paid license service if needed, per-action RBAC, real multi-user server, and independent security review. |
| #111 Backup/restore | Case backup and restore commands preserve DB/WAL/SHM hashes, schema inventory, migration readiness, and restore verification. | Scheduled backups, multi-version migration corpus, and restore drills on production-scale cases. |
| #112-#117 Release/support/training | Release notes, LTS/hotfix policy, support SLA template, training curriculum, quickstart lab, and admin deployment guide are packaged. | CI changelog enforcement, staffed support desk, real training delivery, and deployment proof from customer environments. |
