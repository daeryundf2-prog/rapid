...
The #36~#40 email/cloud rows likewise emit reportability decisions. Email rows remain `email-message-or-mailbox-inventory-triage-pivot` until PST/OST/MSG object decoding, deleted item recovery, threading, duplicates, timezone, attachments, and privilege review are validated. Google Takeout/Gmail rows remain `google-export-triage-pivot` until product matrix coverage, export scope, sidecars, timezone, and provider-native diffs exist. iCloud rows remain `icloud-export-triage-pivot` until account/photo completeness, ADP/shared-album fidelity, and retention/schema behavior are validated. Microsoft 365 rows remain `m365-export-triage-pivot` until eDiscovery, Teams/OneDrive/SharePoint permissions, reactions/attachments, and retention/deleted state are validated. Cloud API rows remain `cloud-api-response-triage-pivot` until OAuth consent/scope, pagination/delta/retry behavior, provider schemas, legal hold, and provider-native comparisons are attached.
