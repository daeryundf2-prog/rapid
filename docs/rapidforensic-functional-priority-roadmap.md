...
Current internal progress: native EVTX rows now attach `evtx_record_provenance` and `evtx_native_parse_profile`, including source/record hashes, record offset/size, chunk boundary context, validation matrix IDs, BinXML status, scalar value counts, decoded type counts, best-effort CDATA/character-reference/entity-reference/processing-instruction token capture, bounded nested `BinXmlType` payload parsing through `evtx-nested-binxml-value-v1`, promoted System/EventData/UserData fields, template IDs, and reportability blockers. Remaining commercial blocker: full BinXML object model coverage and EvtxECmd/Hayabusa record-level diff corpus.
