...
# -*- coding: utf-8 -*-
"""
ingest_dashcam_sources.py

Ingest dashcam videos from folders or E01 (EnCase/EWF) images, then
run the renamer to set filenames from on-screen timestamp (OCR or mixed).

For E01:
- Tries libewf `ewfmount` to expose a raw image (ewf1)
- Prefers The Sleuth Kit (`mmls` + `tsk_recover`) to extract files read-only
- Never modifies source E01; copies only selected video files to destination

Requirements for E01 support:
- macOS/Linux: brew/apt install sleuthkit libewf (commands: ewfmount, mmls, tsk_recover)
- Fallback: if tools missing, the script will skip E01 and print install hints

Usage examples:
  python3 ingest_dashcam_sources.py --src "/path/folderA" --src "/path/image.E01" \
    --dest "/Users/shinyoohag/DashcamImports" --strict-ocr --profiles blackvue,thinkware,generic \
    --max-ocr-per-run 30 --workers 6

After ingest, runs rename_dashcam_videos.py once on the destination root.
"""
...
RENAMER = HERE / "rename_dashcam_videos.py"
...
def mmls_first_filesystem(raw_path: Path) -> Optional[int]:
    try:
        ...
        return None
    # Parse lines like:  000: 0000002048 0000004096  DOS FAT32 (0x0c)
    best_start = None
    best_size = -1
...
def extract_from_e01(e01: Path, stage_dir: Path) -> Optional[Path]:
    ...
    if not have("ewfmount") or not have("mmls") or not have("tsk_recover"):
        ensure_tools_or_warn()
        return None
    mount_dir = stage_dir / "_ewfmount"
    raw_img = mount_dir / "ewf1"
    try:
        mount_dir.mkdir(parents=True, exist_ok=True)
        # Mount E01 -> FUSE raw file
        p = subprocess.run(["ewfmount", str(e01), str(mount_dir)], capture_output=True, text=True)
        if p.returncode != 0 or not raw_img.exists():
            print(f"[error] ewfmount failed for {e01}: {p.stderr.strip()}")
            return None
        # Pick filesystem start sector
        start = mmls_first_filesystem(raw_img)
        if start is None:
            print(f"[error] mmls could not find a FAT/exFAT/NTFS partition in {e01}")
            return None
        extract_dir = stage_dir / "_extract"
        extract_dir.mkdir(parents=True, exist_ok=True)
        # Recover allocated files from that partition
        # tsk_recover -e -a -o <start> raw_img extract_dir
        rec = subprocess.run(["tsk_recover", "-e", "-a", "-o", str(start), str(raw_img), str(extract_dir)], capture_output=True, text=True)
        if rec.returncode != 0:
            print(f"[error] tsk_recover failed: {rec.stderr.strip()}")
            return None
        return extract_dir
    finally:
        # Try to unmount ewf FUSE
        if mount_dir.exists():
            # macOS
...
def run_renamer(dest_root: Path, profiles: List[str], strict_ocr: bool, prefer_start: bool,
    ...
    if not RENAMER.exists():
        print(f"[error] renamer not found: {RENAMER}")
        return
    base_cmd = [sys.executable, str(RENAMER), "--dir", str(dest_root), "--workers", str(workers),
                "--max-ocr-per-run", str(max_ocr)]
    if strict_ocr:
        base_cmd.append("--strict-ocr")
    if prefer_start:
        base_cmd.append("--prefer-start")
    # default: only-unnamed to avoid touching already-dated names
...
def main():
    ...
    for src_s in args.src:
        ...
        if not src.exists():
            ...
            continue
        # Roll-in directory for this source
        stamp = dt.datetime.now().strftime("%Y/%m/%d")
        ...
        if src.is_dir():
            ...
            print(f"[ingest:e01] {src} -> {rollin}")
            ensure_tools_or_warn()
            extract_dir = extract_from_e01(src, rollin / "_stage")
            if extract_dir is None:
                print(f"[skip:e01] could not extract from {src}")
                continue
            copied = collect_videos(extract_dir, rollin)
            print(f"[e01] copied {copied} video(s) from extracted filesystem")
            total_copied += copied
        else:
            print(f"[skip] unsupported source type: {src}")
            continue
        # Also count direct copies
